...
### 2026-07-16, iteration 30 — [[words/野獣|野獣]]

Swadesh #44 also ("wild beast," the sense-narrowed sibling of [[動物]]). Stamped `date-last-perfect: 2026-07-16`. Another clean one — derivation matched (`'yasyu`/야슈/⼘ㄙ⼜ across all three fields, including the visually-unusual `⼘` glyph which is 野's own genuine stored reading, not a typo, confirmed against its character page), and `japanese`/`korean`/`vietnamese` (やじゅう, 야수, dã thú) were all already real, correctly-attested words — no native-substitution bug this time either. **Stand-in note applied**: `characters/獣.md`'s `stand_in` field is `野獣`. Only real defects: blank `hsk_level:` (removed) and a missing `## Notes` section, written from scratch — including a nice literal/figurative double-sense finding ("wild beast" / "brutish person") that's already recorded piecemeal on `獣`'s own character page but had never been pulled together into the word's own encyclopedic entry. `characters/獣.md` is itself unperfected (blank `date-last-perfect`, blank `boundedness`, informal Notes) — flagged for the character sweep. **Resolved 2026-07-19**: perfected directly at the user's request — malformed `japanese_native` YAML (scalar + stray list item) fixed to a proper list, stray `determiner` tag removed, Notes rebuilt to the canonical 4 bullets (the two definition-style lines' "brutish person" sense preserved inside the graphemic bullet's dash-note), `## Words` built from the 4 ground-truth hits (野獣 stand-in, 怪獣, 一角獣, 獣群), no chengyu (波乱万丈's 禽獸 is inside a classical quotation, not the idiom itself), no Derived Characters (`graphemic_classification: 獣` has zero hits). Stamped `2026-07-19`. No homophones.

**Milestone**: clears every swadesh-#44 entry (both 動物 and 野獣 map to the same Swadesh slot). Next: 52 (森林), 54 (果実), continuing ascending.
