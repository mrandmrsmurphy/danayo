...
### 2026-08-02, iteration 496 — [[words/毛|毛]]

Worked from the alphabetical worklist. `characters/毛 (char).md` was already fully perfected content-wise, but had a genuine broken link caught in passing: its own tip-box radical link pointed at "Radical 182," which is actually 飛 ("fly") — 毛's real radical folder is `Radical 082.md`, confirmed by checking both files' own `radical:` frontmatter. Fixed the link (082, not 182 — a simple off-by-100 typo). **Content added to the word page**: `pos: 名詞`; `japanese: もう` (the more general-purpose of the character's two on'yomi, MOU, vs. rarer ぼう); `vietnamese: mao` (the character's only stored reading); `kwin: false` was already present and confirmed correct (諺文 맛 and korean 모 genuinely differ, no change needed); `characters:` bare scalar → list form; dropped the blank `品詞:` field.

Built `## Notes` from scratch: 象形 etymology (feather/hair), the animal-covering compound family (毛皮/羽毛/羊毛) and the phonetic-descendant family that lost the fur/hair meaning entirely (表/耗/芼), Japanese's もう/ぼう split plus the native け (with 髪の毛 showing both layers in one compound), and Vietnamese mao's learned-register-only status vs. native lông. No homophones found (注音 ㄇㄚㄨ shared only by characters — 芼, 矛, 帽, 瑁, 冒 — none with a self-pointing `stand_in`). Stamped `date-last-perfect: 2026-08-02`.

Next (alphabetical): 水 (live-computed candidate — `氏` has no standalone word file at its radical slot; `気`/`气` are the established unlinkable-no-page-component precedent, not a real candidate; `水.md` is the next real, unperfected word file in Kangxi radical-stroke order after `毛` — blank `pos`, stray unrecognized `品詞:` field, bare `## Notes` heading, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-02, iteration 497 — [[words/水|水]]

Worked from the alphabetical worklist. `characters/水 (char).md` was already fully perfected (`stand_in: "水"`, `kwin: true`). **Content added**: `pos: 名詞` (was blank); `品詞:` blank field dropped; `characters:`/`vietnamese` normalized to scalar/list-of-one forms matching this sweep's conventions. `japanese: すい`/`vietnamese: thuỷ`/`kwin: true` were all already present and confirmed correct against the character's own fields — a clean iteration with no bugs found.

Built `## Notes` from scratch: the 象形 flowing-water etymology and its three written forms (bare 水, compressed 氵 beside another component, rare full 氺 in 泰/录/黎); the large concrete-noun compound family (湖水, 淡水, 大水) alongside figurative/technical extensions (風水, 水平, and 水星 "Mercury" continuing the Five-Phases planet-naming pattern also carried by [[金]]/[[火]]/[[木]]); Japanese すい/みず and Vietnamese thuỷ/nước both showing the same Sino-technical-vs-native-everyday split already documented for [[日]]/[[月]]/[[火]] earlier in this sweep; and the vault's own periodic-table mercury abbreviation via [[水銀]], flagged as a genuine cross-CJKV calque rather than a Dan'a'yo-only coinage like [[日素]]/[[月素]]. No homophones (注音 ㄙㄨ shared by ten characters — 樹, 須, 洙, 鬚, 需, 讐, 戍, 殊, 数, 豎 — none with a self-pointing `stand_in`). Stamped `date-last-perfect: 2026-08-02`.

Next (alphabetical): 火 (live-computed candidate — `火.md` is the next real, unperfected word file in Kangxi radical-stroke order after `水` — vietnamese already a 1-item list `hoả`, but bare unglossed `## Notes` numbered list, no `pos`, no `date-last-perfect`; also already cross-references [[火紅素]], the strontium entry sitting at the tail of this sweep's own element-series blank-key-backlog queue). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-02, iteration 498 — [[words/火|火]]

Worked from the alphabetical worklist. `characters/火 (char).md` was already fully perfected (`stand_in: 火`, `kwin: true`). **Real content bug found and fixed**: the word's own `注音` was the literal spelling `ㄏㄨㄚ`, but no syllable page exists at that string — the vault's real canonical syllable page is `ㄏ⺢.md`, using this corpus's own single-glyph rime notation (⺢ = the -ua rime). Corrected to `ㄏ⺢`, matching the character's own stored value — the same character-vs-word `注音` mismatch bug class already on record from `屎`/`破`/`即`/`恩`/`慣`/`調`/`酒` in the vault's history. `羅馬字`/`諺文` (hwa/화) were already correct.

**Content added**: `pos: 名詞`; `japanese: か` (the default of the character's two on'yomi, KA, vs. rarer こ); `vietnamese: hoả` normalized to scalar; `kwin: true` confirmed (諺文/korean both 화). Built `## Notes` from scratch: 象形 etymology and the 灬/灸 compression pattern (paralleling 水's 氵/氺 from last iteration); the folklore compound family (鬼火, 燐火, 火鼠) vs. the technical one (火星 continuing the Five-Phases planet-naming pattern, 火曜日 the fire-weekday); か/こ/ひ (visible even in rendaku'd 花火 hanabi) and Vietnamese hoả/lửa repeating the same Sino-technical-vs-native-everyday split as [[日]]/[[月]]/[[水]]; and [[火紅素]] flagged as a genuine descriptive coinage (from the strontium flame-test color) rather than a cross-CJKV calque like [[水銀]]. No homophones (注音 ㄏ⺢ shared by twelve characters — 禍, 和, 華, 禾, 靴, 踝, 花, 化, 貨, 嘩, 樺 — none with a self-pointing `stand_in`). Stamped `date-last-perfect: 2026-08-02`.

Next (alphabetical): 牛 (live-computed candidate — `爪`/`父`/`爻`/`爿`/`片`/`牙` have no standalone word file at their radical slot; `牛.md` is the next real, unperfected word file in Kangxi radical-stroke order after `火` — blank `vietnamese:`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-02, iteration 499 — [[words/牛|牛]]

Worked from the alphabetical worklist. `characters/牛 (char).md` was already fully perfected (`stand_in: 牛`). **Content added**: `pos: 名詞` (was missing entirely, not just blank); `vietnamese: ngưu` (picked from the character's own 3-variant list — ngâu, ngõ, ngưu — confirmed as the reading actually alive in shared cultural reference via Ngưu Lang Chức Nữ, the Vietnamese form of the Cowherd/Weaver-Girl legend already referenced in this word's own Words list via 牛郎星/牛郎); `japanese: ぎゅう` (the character's only on'yomi, GYUU); `kwin: false` already present, confirmed correct.

**Real homophone found**: `注音 ㄋ⼜`/`뉴` is also carried by [[柔]] ("tender, soft, weak"), an existing but unperfected standalone word. Added the full callout to `牛`'s now-complete page and a matching anticipatory one to `柔` (mirroring the `亙`/`尋`/`痩`/`銹`/`投`/`豆` pattern from earlier in this sweep). Built `## Notes` from scratch: 象形 etymology (frontal ox-head pictograph), the literal livestock family (牛乳, 水牛) vs. the loose-resemblance extensions to unrelated animals (犀牛 rhinoceros, 海牛 manatee — a shared English calque, 蝸牛 snail, where 牛 evokes slowness/antennae rather than anything bovine), the cross-CJKV Cowherd/Weaver-Girl astronomical legend (牛郎星/Altair) as the tiebreaker for the Vietnamese reading choice, and ぎゅう/うし with the note that 牛丼 gyūdon names a beef dish without any dedicated "beef" sense splitting off from "cow" itself. Stamped `date-last-perfect: 2026-08-02`.

Next (alphabetical): 玄 (live-computed candidate — `犬` was already perfected; `玄.md` is the next real, unperfected word file in Kangxi radical-stroke order after `牛` — literal `vietnamese: null`, no `pos`, bare `# Notes` placeholder holding only a stray `[[Radical 095]]` bullet, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-02, iteration 500 — [[words/玄|玄]]

Milestone: the 500th iteration of this word-perfecting sweep. Worked from the alphabetical worklist. **Unusual for this sweep**: `characters/玄 (char).md` turned out to be genuinely unperfected itself (no `date-last-perfect` at all) — its own `## Notes` is just a bare `[[Radical 095]] (玄)` bullet with no real graphemic write-up, and its `## Words` section is malformed (unruby'd, mostly ungloss'd, with two stray Middle-Chinese-initial/final wikilinks floating outside any bullet at the very end). Left that page untouched and flagged for the separate character-perfection sweep ([[Loop Work.md]]) rather than attempting a full character rebuild inside a word-creation cycle — but its individual scalar fields (mandarin/cantonese/japanese/korean/vietnamese) were still reliable ground truth and used as such here.

**Content added to the word page**: `pos: 性詞` (matching the character); `japanese: げん` (the more general-purpose of the character's two on'yomi, GEN, vs. rarer けん); `vietnamese: huyền` (the character's only stored reading); `kwin: false` (was missing the field entirely — Dan'a'yo 훤 vs Sino-Korean 현 genuinely differ in onset). Built `## Notes` from scratch: the twisted-black-red-silk etymology behind [[Radical 095]], the color-to-abstract-profundity semantic shift (culminating in the Dao De Jing's 玄之又玄), and 玄武/玄米 as the two concrete survivors of the original color sense — both already thinly present on the character's own unfinished Words list. Japanese げん/けん and native くろ ("black," tying straight back to the original dye color) and Vietnamese huyền (leaning abstract, like Japanese, with native đen carrying the literal color) round out the cross-CJKV picture.

**Real homophone group found — a third three-way group this sweep**: `注音 ㄏ⼔ㄋ`/`훤` is also carried by two other existing standalone words, [[懸]] ("hang, suspend, hoist") and [[県]] ("county, district, prefecture"). Added the full mutual callout across all three pages, matching the anticipatory-touch pattern used throughout this sweep (`亙`/`尋`/`痩`/`銹`/`投`/`豆`/`柔`). Stamped `date-last-perfect: 2026-08-02` on `玄` only.

Next (alphabetical): 玉 (live-computed candidate — `玉.md` is the next real, unperfected word file in Kangxi radical-stroke order after `玄` — blank `vietnamese:`, no `pos`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-02, iteration 501 — [[words/玉|玉]]

Worked from the alphabetical worklist. `characters/玉 (char).md` was already fully perfected (`stand_in: "玉"`). **Content added**: `pos: 名詞`; `japanese: ぎょく` (the default of the character's two on'yomi, GYOKU, vs. rarer ごく — the character's own `japanese_native: ø` is flagged as a likely real data gap, since だま/たま is an extremely common native reading in ordinary words like 玉葱/目玉, the same `ø`-despite-clear-native-life pattern already found on [[手]] earlier in this sweep); `vietnamese: ngọc` (the character's only stored reading); `kwin: false` (field was missing entirely — Dan'a'yo 욕 and Sino-Korean 옥 share the same MC checked coda but diverge in the vowel, Dan'a'yo keeping a y-glide Korean lacks).

Built `## Notes` from scratch: the 象形 etymology (a ritual 琮 jade totem, or alternatively three jade pieces strung on a cord), jade's ceremonial/aristocratic prestige carrying directly into figurative "precious, flawless" extensions (金科玉律 pairing it with 金 as the two most prestige-laden materials in classical idiom), and the genuinely opaque modern compound 玉葱 "onion" (jade's lustrous-sheen association extended to an onion's layers, unrelated to any stone sense). No homophones (注音 ⼄ㄎ shared only by characters — 獄, 欲, 浴 — none with a self-pointing `stand_in`). Stamped `date-last-perfect: 2026-08-02`.

Next (alphabetical): 瓦 (live-computed candidate — `瓜` has no standalone word file at its radical slot; `瓦.md` is the next real, unperfected word file in Kangxi radical-stroke order after `玉` — already carries a `>[!warning] Homophones` callout to [[臥]], but blank `vietnamese:`, no `pos`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 502 — [[words/瓦|瓦]]

Worked from the alphabetical worklist. `characters/瓦 (char).md` was already fully perfected (`stand_in: 瓦`, `kwin: true`); the reciprocal homophone [[臥]] was also already fully perfected with its own half of the callout already in place — only `瓦`'s own page was the gap. **Content added**: `pos: 名詞`; `japanese: が` (the character's only on'yomi, GA); `kwin: true` (field was missing entirely — 諺文/korean both 와, confirmed matching); `vietnamese: ngói` — a real judgment call, picked over the character's other two stored variants (ngoa, ngoã) as the form actually alive in everyday Vietnamese (nhà ngói, "tile-roofed house"), documented explicitly as a formal-vs-nativized-descendant choice rather than an arbitrary pick.

Built `## Notes` from scratch: 象形 etymology (fired pottery / a curved roof tile — the character is itself [[Radical 098|瓦]]), the unrelated modern loan-sense as the watt unit of power (瓦特, distinct from [[佤]] "Wa," a genuine phonetic derivative sharing the syllable rather than a coincidental homophone), Japanese が/かわら with 瓦解 "to collapse" as the idiom-turned-ordinary compound, and the ngoa/ngoã/ngói three-way Vietnamese split motivating the reading choice above. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 甘 (live-computed candidate — `甘.md` is the next real, unperfected word file in Kangxi radical-stroke order after `瓦` — blank `vietnamese:`, no `pos`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 503 — [[words/甘|甘]]

Worked from the alphabetical worklist. `characters/甘 (char).md` was already fully perfected, including its own already-documented `graphemic_classification` correction (象形→指事, per Wiktionary, since 甘 is the ancestor form of [[含]] via the same "something in the mouth" image rather than a directly-pictured object). **Content added**: `pos: 性詞`; `japanese: かん` (the character's only on'yomi, KAN); `vietnamese: cam` (the character's only stored reading — noted in passing that modern Vietnamese cam is also the everyday word for "orange," an unrelated homophone collision); `kwin: true` confirmed (諺文/korean both 감).

**Real homophone found, and an unusually direct one**: `注音 ㄍㄚㄇ`/`감` is also carried by [[紺]] ("navy blue") — which is not just a coincidental homophone but one of 甘's own listed Derived Characters (a genuine phonetic descendant), the first time in this sweep the homophone relationship and the phonetic-derivation relationship have been the same underlying fact rather than two independent coincidences. Added the full callout to `甘` and a matching anticipatory one to `紺`, phrased to flag this distinction explicitly rather than presenting it as an ordinary coincidental convergence like the sweep's other homophone groups. Built `## Notes` from scratch: the 指事 mouth-with-sweetness etymology, the taste-to-attitude semantic generalization (甘心) contrasted with this vault's own compound family staying mostly literal (甘露/甘味/甘藷/甘蕉), and かん/あまい plus cam's dual "sweet-compound / orange" life in Vietnamese. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 白 (live-computed candidate — `生`/`用`/`田`/`疋` etc. have no standalone word file at their radical slot; `白.md` is the next real, unperfected word file in Kangxi radical-stroke order after `甘` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 504 — [[words/白|白]]

Worked from the alphabetical worklist. `characters/白 (char).md` was already fully perfected (`stand_in: "白"`). **Content added**: `pos: 名詞`; `japanese: はく` (the default of the character's two on'yomi, HAKU, vs. びゃく's Buddhist/classical niche); `vietnamese: bạch` (picked over the character's other stored reading, bạc — which has drifted to mean "silver" as its own distinct word in modern Vietnamese, not a plain synonym); `kwin: false` (field was missing entirely — Dan'a'yo 박 vs Sino-Korean 백 diverge in the vowel).

**Real homophone found, previously undocumented on an already-long-perfected page**: `注音 ㄅㄚㄎ`/`박` is also carried by [[百]] ("hundred") — perfected since 2026-06-03 but missing the callout (and, found in passing, missing its `>[!tip]` disambiguation banner entirely, a real structural gap on an otherwise-stamped page). Added the full callout plus the missing tip box to `百`, and the reciprocal callout to `白`. Built `## Notes` from scratch: the two competing 象形 origin theories (acorn vs. stylized head) Wiktionary itself leaves unsettled, the literal-color vs. clarity/emptiness figurative split (明白/空白), 太白星 (Venus) continuing this vault's Five-Phases planet-naming family, はく/びゃく/しろい, and the bạch/bạc split cognate with [[白金]]. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 皿 (live-computed candidate — `皮` has no standalone word file at its radical slot; `皿.md` is the next real, unperfected word file in Kangxi radical-stroke order after `白` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 505 — [[words/皿|皿]]

Worked from the alphabetical worklist. **`characters/皿 (char).md` turned out to be genuinely unperfected itself** (blank `pos: ""`, `## Notes` holding only two dangling, un-bulleted Middle-Chinese-initial/final wikilinks, no `date-last-perfect`) — the second time this sweep has hit this situation (after [[玄]]). Flagged for the separate character-perfection sweep rather than fixed here, though the character's scalar fields remained trustworthy for this word page.

**Content added**: `pos: 名詞`; `japanese: ばい` (nominally the first-listed of the character's two on'yomi, BAI, though flagged in Notes as barely used at all — native さら dominates completely, a more lopsided native/Sino split than any other character in this sweep); `vietnamese: mãnh` — verified via web search against Hán Việt reference dictionaries rather than guessed, since the character's own page stores five variants (mãnh, mạnh, mảng, mảnh, mịn) and only mãnh is actually the reading for this "dish/container" sense — the other four are real Vietnamese words belonging to unrelated homophone characters, the same stored-field-contamination pattern already seen elsewhere in this vault; `kwin: true` confirmed (諺文/korean both 명).

**Real homophone group found — a fourth three-way group this sweep**: `注音 ㄇ⼶ㄫ`/`명` is also carried by two other existing standalone words, [[明]] ("bright") and [[鳴]] ("cry, chirp"). Added the full mutual callout across all three pages. Built `## Notes` from scratch, noting 皿 stays almost entirely literal (a plain container/vessel radical) without the rich figurative afterlife seen on [[日]]/[[月]]/[[白]] earlier in this sweep. Stamped `date-last-perfect: 2026-08-03` on `皿` only.

Next (alphabetical): 矢 (live-computed candidate — `目` was already perfected; `矛` has no standalone word file at its radical slot; `矢.md` is the next real, unperfected word file in Kangxi radical-stroke order after `皿` — already carries a `>[!warning] Homophones` callout to [[四]]/[[視]], vietnamese already a 1-item list `thỉ`, but bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 506 — [[words/矢|矢]]

Worked from the alphabetical worklist. **A third character page found genuinely unperfected in this sweep** ([[玄]], [[皿]] were the first two): `characters/矢 (char).md` has no `date-last-perfect`, and its `## Notes` holds only two dangling CC-initial/final wikilinks plus a leftover "Components:"-style bullet naming 𠂉/大, never rewritten into a real graphemic write-up. Flagged for the separate character sweep rather than fixed here.

**Content added**: `pos: 名詞`; `japanese: し` (the character's only on'yomi, SHI); `kwin: false` (field was missing — Dan'a'yo 싀 vs Sino-Korean 시 diverge in the vowel); `vietnamese: thỉ` and the existing `>[!warning] Homophones` callout to [[四]]/[[視]] were both already correct and complete — double-checked against a full reading-sweep (師, 肆, 死, 獅, 諡, 氏, 私 also share this syllable but none has a self-pointing `stand_in`, confirming no third homophone word is missing). Built `## Notes` from scratch: 象形 etymology (fletching over an arrowhead), 矢's narrower classical register relative to modern 箭 (matching the arrow sense of 矢志), and し/や plus Vietnamese thỉ's learned-only status against the native mũi tên. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 石 (live-computed candidate — `示` has no standalone word file at its radical slot; `石.md` is the next real, unperfected word file in Kangxi radical-stroke order after `矢` — most fields already filled including a stray duplicate `品詞:`, unglossed numbered `## Notes` list, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 507 — [[words/石|石]]

Worked from the alphabetical worklist. `characters/石 (char).md` was already fully perfected. **Incidental bug fixed while cross-checking fields**: the character's own `japanese_native` had a malformed duplicate — a clean scalar `いし` followed by a stray extra list-item line repeating the same value — collapsed to a single scalar. **Content added to the word page**: `pos: 名詞`/`品詞:` duplicate dropped, kept as single `pos`; `japanese: せき`/`vietnamese: thạch` normalized to scalars (already correct); `kwin: true` confirmed (諺文/korean both 석). Rebuilt `## Notes` from a bare unglossed numbered list into full prose: 象形 etymology (a stone beneath the same 厂 cliff-radical already documented on [[雁]]/[[反]], later distorted to 丆), the mineral/geological compound family, [[石素]] as a Dan'a'yo-internal lithium coinage alongside [[日素]]/[[月素]]/[[火紅素]], and せき/いし plus Vietnamese thạch/đá repeating this sweep's recurring Sino-technical/native-everyday split.

**Real homophone found**: `注音 ㄙㄝㄎ`/`석` is also carried by [[惜]] ("begrudge, rue"), an existing but unperfected standalone word — added the full callout to `石` and a matching anticipatory one to `惜`. Checked the remaining `ㄙㄝㄎ`-reading characters (析, 碩, 適, 錫, 釈, 席, 汐, 昔, 夕, 潟) — none has a self-pointing `stand_in`, confirming no third homophone word exists at this reading. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 禾 (live-computed candidate — `示`/`禸` have no standalone word file at their radical slot; `禾.md` is the next real, unperfected word file in Kangxi radical-stroke order after `石` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`; its stored `注音` is the literal spelling `ㄏㄨㄚ`, the same non-canonical-vs-`ㄏ⺢` bug class already fixed on [[火]], worth re-checking against the character's own field). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 508 — [[words/禾|禾]]

Worked from the alphabetical worklist. Fixed the flagged `注音` bug (`ㄏㄨㄚ` → `ㄏ⺢`) and, while checking for homophones at the corrected reading, uncovered a real three-way group this sweep had been carrying incompletely: [[化]] (perfected 2026-07-26) had already correctly identified [[火]] and this word as homophones and left a forward note promising the reciprocal callouts once each came up — but both `禾` and `火` had themselves been stuck on the same non-canonical `ㄏㄨㄚ` spelling, so a plain-string search never actually surfaced them as matches until this iteration corrected the spelling on all three. Fixed `化`'s own `注音` to `ㄏ⺢` too, added the missing reciprocal callout retroactively to `火` (perfected in iteration 498, two iterations ago, but never given its half of this callout), and completed the group on `禾`. All three — 火/化/禾 — now correctly cross-link each other.

`characters/禾 (char).md` is a fourth character page found genuinely unperfected this sweep (after [[玄]]/[[皿]]/[[矢]]): blank `pos`, `## Notes` holding only dangling CC-links plus one bare compound bullet, no `date-last-perfect` — flagged for the character sweep. **Content added to the word page**: `pos: 名詞`; `japanese: か` (the character's only on'yomi, overshadowed by native いね for the living plant); `vietnamese: hoà` (the character's only stored reading); `kwin: true` confirmed. Built `## Notes` covering the 象形 grain-stalk etymology, か/いね, and Vietnamese hoà's learned-only register. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 竹 (live-computed candidate — `竹.md` is the next real, unperfected word file in Kangxi radical-stroke order after `禾` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 509 — [[words/竹|竹]]

Worked from the alphabetical worklist. `characters/竹 (char).md` was already fully perfected. **Incidental fix**: the same malformed-duplicate `japanese_native` bug already found on [[石]] (scalar たけ followed by a stray repeated list-item line) — collapsed to a single scalar. **Content added to the word page**: `pos: 名詞`; `japanese: ちく` (the character's only on'yomi); `vietnamese: trúc` (the character's only stored reading); `kwin: true` (field was missing entirely — 諺文/korean both 죽, confirmed matching).

**Real homophone found**: `注音 ㄐㄨㄎ`/`죽` is also carried by [[粥]] ("congee"), an existing but unperfected standalone word — added the full callout to `竹` and a matching anticipatory one to `粥`. Checked the remaining `ㄐㄨㄎ`-reading characters (祝, 築, 竺, 賊, 則) — none has a self-pointing `stand_in`, confirming no third homophone word exists at this reading. Built `## Notes` from scratch: 象形 etymology (two drooping bamboo stalks), the botanical compound family alongside the emblematic 松竹梅 "Three Friends of Winter" motif shared across Chinese/Japanese/Korean New Year iconography, and ちく/たけ plus Vietnamese trúc's literary-register status. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 米 (live-computed candidate — `米.md` is the next real, unperfected word file in Kangxi radical-stroke order after `竹` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 510 — [[words/米|米]]

Worked from the alphabetical worklist. `characters/米 (char).md` was already fully perfected. **Content added**: `pos: 名詞`; `japanese: まい` (picked from an unusually wide three-reading set — まい ties to the rice sense in 玄米/白米, べい covers both the unrelated metric "meter" sense and the transliteration character for "America" in 米国, め is rarest/classical, all documented); `vietnamese: mễ` (over the character's other stored reading, mè — which is actually the modern Vietnamese word for "sesame," an unrelated homophone contamination, not a real variant of 米); `kwin: false` (field was missing — Dan'a'yo 메 vs Sino-Korean 미 diverge in the vowel).

**Real homophone group found — a genuine phonetic family, not just coincidental**: `注音 ㄇㄝㄧ`/`메` is also carried by two other existing standalone words, [[迷]] ("be lost, be confused") — a direct phonetic descendant already listed on 米's own Derived Characters — and [[謎]] ("riddle, mystery, enigma"), built on 迷 as its own phonetic component one further step removed. Added the full mutual callout across all three pages. Built `## Notes` from scratch: the 象形 scattered-rice-grain etymology, [[費米金]] (fermium, a transliteration coinage distinct from this vault's descriptive element names), and the meter/America homophone tangent riding on the same syllable with no semantic link to rice. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 老 (live-computed candidate — `糸`/`缶`/`网`/`羊`/`羽` etc. have no standalone word file at their radical slot; `老.md` is the next real, unperfected word file in Kangxi radical-stroke order after `米` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 511 — [[words/老|老]]

Worked from the alphabetical worklist. `characters/老 (char).md` was already fully perfected. **Real content bug found and fixed**: the word's own `korean` field was 노 (the South Korean 두음법칙-shifted form, initial ㄹ regularized to ㄴ), violating the vault's own standing rule that `korean` always records the North Korean/문화어 pronunciation — corrected to 로, matching the character's own already-correct field, per [[feedback_korean_reading_north]]. `vietnamese: lão` picked from an unusually noisy 8-item stored list (láu, lão, lảo, lảu, lẩu, lẽo, lếu, rảu) — lão is the one independently well-attested for this sense (lão nhân, lão hóa, and Lão Tử directly paralleling this vault's own 老子), the rest almost certainly homophone contamination. `pos: 名詞`/`japanese: ろう`/`kwin: false` added.

Built `## Notes` from scratch: the 会意 etymology (man + hair + cane), the character's own already-documented 転注 relationship with [[考]] as a classic philology textbook example, the 老子/老師/老爺 title-and-name compound family, and ろう alongside a narrower bound native root お- (老いる) rather than a free native word — closer to [[矢]]'s classical-register profile than a full native/Sino split. No homophones (注音 ㄌㄚㄨ shared only by characters — 𢭐, 労, 牢 — none with a self-pointing `stand_in`). Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 肉 (live-computed candidate — `而`/`耳` were already perfected; `耒`/`聿` have no standalone word file at their radical slot; `肉.md` is the next real, unperfected word file in Kangxi radical-stroke order after `老` — blank `vietnamese:`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 512 — [[words/肉|肉]]

Worked from the alphabetical worklist. `characters/肉 (char).md` was already fully perfected. **Content added**: `pos: 名詞`; `japanese: にく` (overwhelmingly dominant of the character's two on'yomi, vs. rare じゅう — にく itself functions as the ordinary everyday word, no separate native 訓読み exists at all, an unusual case in this sweep); `vietnamese: nhục` (the character's only stored reading); `kwin: false` already present, confirmed correct.

Built `## Notes` from scratch: the 象形 rib/meat-slice etymology and its 月-stylization beside phonetic components (graphically identical to, but semantically unrelated to, the "moon" 月 covered earlier in this sweep); the culinary compound family alongside flesh-as-kinship (骨肉, echoing the Biblical "bone of my bone") and flesh-as-vulnerability (弱肉強食) figurative extensions; 苦肉 and this vault's own 道成肉身 as strategic/doctrinal registers respectively. `kwin` false confirmed via the same 日母-onset-loss pattern already documented on [[日]] earlier in this sweep. No homophones (注音 ㄋㄨㄎ shared only by 衂, a `名専字`-restricted character). Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 臼 (live-computed candidate — `臣`/`自` have no standalone word file at their radical slot; `至` was already perfected; `臼.md` is the next real, unperfected word file in Kangxi radical-stroke order after `肉` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 513 — [[words/臼|臼]]

Worked from the alphabetical worklist. **A fifth character page found genuinely unperfected this sweep** ([[玄]]/[[皿]]/[[矢]]/[[禾]] were the first four): `characters/臼 (char).md` has only two dangling CC-initial/final wikilinks under `## Notes`, no `date-last-perfect`. Flagged for the character sweep. **Content added**: `pos: 名詞`; `japanese: きゅう` (the character's only on'yomi, rare in ordinary vocabulary vs. the fully productive native うす); `vietnamese: cối` (the everyday living word — cối xay, cối giã — over the character's other stored reading cữu, an unrelated classical sense); `kwin: false` (Dan'a'yo 귯 retains an MC coda Sino-Korean 구 has lost).

**Real homophone group found**: `注音 ㄍ⼜ㄛ`/`귯` is also carried by [[旧]] ("paleo-, former, old") and [[糾]] ("twist, investigate"), both existing but unperfected standalone words. Added the full mutual callout across all three pages. **Incidental fix while there**: `旧`'s own `korean` field was the literal string `"null"`, the same serialization-artifact bug already found on [[奥]] earlier in this sweep — corrected to 구, matching `旧 (char)`'s own field. Checked 求/舅, the remaining `ㄍ⼜ㄛ`-reading characters — neither has a self-pointing `stand_in`, confirming no fourth homophone word exists. Built `## Notes` from scratch: the 象形 mortar etymology and its anatomical "socket" extension, paralleling [[斗]]'s earlier shape-to-metaphor move. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 舌 (live-computed candidate — `舌.md` is the next real, unperfected word file in Kangxi radical-stroke order after `臼` — blank `vietnamese:`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 514 — [[words/舌|舌]]

Worked from the alphabetical worklist. **Real `kwin` bug found and fixed on two pages**: both this word's own page and `characters/舌 (char).md` were stamped `kwin: true`, but Dan'a'yo `諺文: 섣` (SEOD) and Sino-Korean `설` (SEOL) are different Unicode syllables (ㄷ vs ㄹ coda) — corrected both to `kwin: false`, the same MC-checked-coda-vs-Korean-liquid pattern already documented on [[月]]/[[越]] earlier in this sweep. **Content added**: `pos: 名詞`; `japanese: ぜつ` (the dominant of two on'yomi, vs. rarer せつ); `vietnamese: thiệt` (picked over the character's other stored variant thịt, which is actually the ordinary Vietnamese word for "meat, flesh" — an unrelated homophone contamination, the same stored-field-noise pattern seen repeatedly this sweep).

Built `## Notes` from scratch: the 象形 forked-snake-tongue etymology, the character's own already-documented unlinkable-ancestor phonetic family (𠯑 → 活/刮/括/筈/蛞/聒, the same pattern as 悟/五 and 栽/才), and the anatomical/speech extensions (舌根 as both tongue-root and taste faculty, 舌戦/毒舌 as speech-register compounds). **Real homophone found**: `注音 ㄙㄝㄊ`/`섣` is also carried by [[屑]] ("scraps, waste tidbits"), an existing but unperfected standalone word — added the full callout to `舌` and a matching anticipatory one to `屑`. Checked the remaining `ㄙㄝㄊ`-reading characters (設, 洩, 偰, 楔, 禼, 薛) — none has a self-pointing `stand_in`. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 血 (live-computed candidate — `舛`/`舟`/`艮`/`色`/`艸`/`虍`/`虫` etc. have no standalone word file at their radical slot; `血.md` is the next real, unperfected word file in Kangxi radical-stroke order after `舌` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 515 — [[words/血|血]]

Worked from the alphabetical worklist. `characters/血 (char).md` was already fully perfected. **Content added**: `pos: 名詞`; `japanese: けつ` (the character's only on'yomi); `vietnamese: huyết` (the formal/medical register, over the character's other genuinely-attested variant tiết, a colloquial culinary-register word for blood-as-ingredient — both real, huyết picked as the register matching this word's plain "blood" gloss; the third stored candidate hoét shows no comparable independent life and was flagged as likely noise); `kwin: false` already present, confirmed correct.

Built `## Notes` from scratch: the 会意 etymology (a drop of blood in a sacrificial vessel, 一+皿), noting this is one of the rare cases in this sweep where the character's oldest ritual sense and its modern figurative idiom (血誓盟約, "a covenant in blood") point at the same underlying image rather than having drifted apart, plus the medical/folklore compound family (血液, 出血, 吸血鬼) and けつ/ち. No homophones (注音 ㄏ⼔ㄊ shared only by [[穴]], whose own `stand_in` is 洞穴, not itself). Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 行 (live-computed candidate — `行.md` is the next real, unperfected word file in Kangxi radical-stroke order after `血` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 516 — [[words/行|行]]

Worked from the alphabetical worklist. `characters/行 (char).md` was already fully perfected. **Content added**: `pos: 事詞` (matching the character); `japanese: こう` (the motion/institution reading, vs. ぎょう's religious/ceremonial register and rare あん — the character's own `japanese_native: ø` flagged as a real gap for so productive a character); `vietnamese: hành` — a genuine judgment call among the character's unusually wide 6-item stored set (hàng, hành, hãng, hăng, hạnh, ngành), which reflects real cross-sense polysemy rather than pure noise (行 itself splits xíng "go/act" vs. háng "row/firm" in Chinese); hành matches this word's specific "attend, go to" gloss.

Built `## Notes` from scratch: the 指事 crossroads etymology, the motion/abstract-action compound families (旅行/銀行 vs. 行動/執行/施行), 行星's shared "wandering star" etymology with the English word "planet," and the row/business historical sense underlying 銀行/五行 that also explains the wide Vietnamese/Japanese reading splits. **Real homophone group found**: `注音 ㄏㄚㄫ`/`항` is also carried by [[桁]] ("beam," already listed as one of 行's own Derived Characters — a genuine phonetic descendant) and [[項]] ("nape, item"). Added the full mutual callout across all three pages. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 角 (live-computed candidate — `衣`/`襾`/`見` etc. have no standalone word file at their radical slot or were already perfected; `角.md` is the next real, unperfected word file in Kangxi radical-stroke order after `行` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 517 — [[words/角|角]]

Worked from the alphabetical worklist. `characters/角 (char).md` was already fully perfected. **Content added**: `pos: 名詞`; `japanese: かく` (dominant of three on'yomi for the geometric sense, vs. rare ろく/こく); `vietnamese: giác` (the standard reading for the geometric sense — tam giác "triangle," directly paralleling 三角 — picked over five noisier stored variants, one of which, góc, is actually the native Vietnamese word for "corner," parallel to Japanese's own native かど rather than a genuine Sino-Vietnamese doublet); `kwin: false` (Dan'a'yo 곡 vs Sino-Korean 각 diverge in the vowel).

Built `## Notes` from scratch: the 象形 animal-horn etymology, the horn→angle semantic extension anchoring the geometric compound family (三角/直角/五角形/七角形), and 角逐's figurative "locking horns" extension. **Real homophone found**: `注音 ㄍㄛㄎ`/`곡` is also carried by [[谷]] ("valley") — added the full callout to `角` and a matching anticipatory one to `谷` (itself next in this sweep's own radical-order queue). Checked 穀/国, the remaining characters sharing this reading — neither has a self-pointing `stand_in`. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 谷 (live-computed candidate — the next real, unperfected word file in Kangxi radical-stroke order after `角` — already carries this iteration's freshly-added `>[!warning] Homophones` callout to [[角]], but literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 518 — [[words/谷|谷]]

Worked from the alphabetical worklist. `characters/谷 (char).md` was already fully perfected (`aliases: [穀]` — a genuine oddity, since 穀 also carries its own independent character page with its own separate `stand_in`, an alias-vs-independent-page tension flagged here but left for the character sweep, not touched). **Content added**: `pos: 名詞`; `japanese: こく` (the more common of two on'yomi, vs. rare よく — the character's own `japanese_native: きわ` "edge, brink" is a distinct concept from the ordinary word for valley, たに, whose absence from the field is flagged as a likely real gap); `vietnamese: cốc` (the standard Sino-Vietnamese reading, sơn cốc — picked over góc, the native word for "corner" and the same contamination pattern just found on [[角]] one iteration ago, and hốc, a related-but-distinct "hollow, cavity" sense); `kwin: true` confirmed (諺文/korean both 곡).

Built `## Notes` from scratch: the 象形 water-between-slopes etymology, and the character's own wide phonetic-only descendant family (俗/容/浴/欲/裕, none retaining any trace of the valley sense) already documented on its own page. Stamped `date-last-perfect: 2026-08-03`, completing the [[角]]/谷 homophone pair from last iteration.

Next (alphabetical): 豆 (live-computed candidate — the next real, unperfected word file in Kangxi radical-stroke order after `谷` — already carries its `>[!warning] Homophones` callout to [[斗]]/[[投]] from earlier in this sweep (iteration 492), but blank `vietnamese:`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 519 — [[words/豆|豆]]

Worked from the alphabetical worklist, closing out the [[斗]]/[[投]]/豆 three-way homophone group opened back in iteration 492. `characters/豆 (char).md` was already fully perfected, including its own explicit flag that 豆's 象形 origin (some kind of vessel) was purely phonetically borrowed for "bean," displacing genuine Old Chinese 菽, and that 豆 is unrelated to the visually similar bottom portion of [[豊]] — a false-friend warning worth repeating on the word page too. **Content added**: `pos: 名詞`; `vietnamese: đậu` (the standard, thoroughly modern reading — đậu phụ, đậu nành — over the character's other stored variant dấu, actually the unrelated Vietnamese word for "mark, sign"); `japanese: とう` already present, confirmed correct — and, unusually for this sweep, its counterpart ず (ZU, 大豆 daizu) is genuinely equally productive rather than a rare/dominant asymmetry, documented as such rather than dismissed.

Built `## Notes` from scratch: the borrowed-pictograph etymology, the purely phonetic (not semantic) descendant family (頭/痘/豎/逗/短), and とう/ず/まめ. `kwin` false confirmed, sharing the same MC-coda-preservation pattern already noted on this word's own homophones [[斗]]/[[投]]. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 走 (live-computed candidate — `豕`/`豸`/`貝` have no standalone word file at their radical slot; `赤` was already perfected; `走.md` is the next real, unperfected word file in Kangxi radical-stroke order after `豆` — most fields already filled including a stray duplicate `品詞:`, bare `## Notes` heading, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 520 — [[words/走|走]]

Worked from the alphabetical worklist. `characters/走 (char).md` was already fully perfected. **Content added**: `pos: 事詞` (matching the character; `品詞:` duplicate dropped); `vietnamese: tẩu` (the Sino-Vietnamese reading alive in đào tẩu "to flee" — over the character's other stored reading rảo, a real but related-not-identical "walk briskly" sense); `japanese: そう`/`kwin: false` already present, confirmed correct.

**Real homophone found, with a genuine false positive ruled out along the way**: `注音 ㄙㄛㄨ` is also stored on [[小雨]] ("light rain, drizzle"), but a closer check showed its own `羅馬字`/`諺文` (so'u/소우, the two-morpheme 小+雨 reading) actually differ from 走's (sou/솟) — the bopomofo string alone coincidentally collides without the underlying Dan'a'yo phonology matching, so per the checklist's own "same 注音 / 羅馬字" criterion 小雨 doesn't qualify and was excluded. The real homophone is [[少]] ("few, scarce") — already perfected since 2026-06-29 but missing this callout — added to both pages. Built `## Notes` from scratch: the 象形 running-figure-over-止 etymology, 東奔西走 as the reduplicated idiom extension, and そう/はしる. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 足 (live-computed candidate — `足.md` is the next real, unperfected word file in Kangxi radical-stroke order after `走` — most fields already filled including a stray duplicate `品詞:`, bare `## Notes` heading, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 521 — [[words/足|足]]

Worked from the alphabetical worklist. `characters/足 (char).md` was already fully perfected. **Content added**: `pos: 名詞` (matching the character; `品詞:` duplicate dropped); `vietnamese: túc` (the character's only stored reading); `japanese: そく`/`kwin: true` already present, confirmed correct.

Built `## Notes` from scratch: the 会意 etymology — 口 as a knee/joint shape (not "mouth") over [[止]], the whole leg from knee to foot, not just the foot the modern gloss suggests — the anatomical compound family alongside the foot-to-sufficiency metaphor (満足, 自給自足) recurring across the sphere, and 画蛇添足 using the literal anatomical sense as its idiom's punchline. そく/あし plus Vietnamese túc/chân repeating this sweep's recurring Sino-technical/native-everyday pattern. No homophones (注音 ㄐㄛㄎ shared only by characters — 擢, 属, 族, 嘱 — none with a self-pointing `stand_in`). Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 金 (live-computed candidate — `身`/`辛`/`辰`/邑/酉/釆 etc. have no standalone word file at their radical slot; `車`/`里` were already perfected; `金.md` is the next real, unperfected word file in Kangxi radical-stroke order after `足` — most fields already filled including a stray duplicate `品詞:`, malformed tip-box intro missing the character name, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 522 — [[words/金|金]]

Worked from the alphabetical worklist. **A sixth character page found in a non-current-checklist state this sweep**: `characters/金 (char).md` carries `date-last-perfect: 2026-03-12`, among the oldest surviving stamps in this vault, structurally out of step with the current checklist (SKIP/MC bullets merged into one line, two CC-initial/final wikilinks stray inside `## Words` rather than under `## Notes`) — flagged for the character-perfection sweep rather than rebuilt here; its scalar fields remained reliable. Fixed the malformed tip-box intro ("This is about the word." missing 金's name) on the word page itself.

**Content added**: `pos: 名詞` (`品詞:` duplicate dropped); `japanese: きん`/`vietnamese: kim`/`kwin: true` all already present, confirmed correct. Folded the pre-existing disambiguation bullets (cross-references to [[黄金]]/[[金属]]) into the proper opening Notes bullet per this vault's stand-in-note convention, rather than leaving them as a separate ad-hoc list.

**Real homophone found, already anticipated**: `注音 ㄍㄧㄇ`/`김` is also carried by [[今]] ("now"), perfected back on 2026-07-26 with a forward note promising this exact reciprocal callout — both characters share the same Old Chinese phonetic ancestor 亼 per 金's own character-page etymology, a genuine phonetic-family relationship rather than coincidence. Added the callout here and updated 今's stale forward-reference sentence. Checked 禁/襟, the remaining `ㄍㄧㄇ`-reading characters — neither has a self-pointing `stand_in`. Built out the rest of `## Notes`: the copper→metal→gold semantic narrowing history already documented on the character page, and 金 as Korea's most common surname, a genuinely separate onomastic layer. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 長 (live-computed candidate — `門`/`隶`/`隹` have no standalone word file at their radical slot; `長.md` is the next real, unperfected word file in Kangxi radical-stroke order after `金` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 523 — [[words/長|長]]

Worked from the alphabetical worklist. **A seventh character page found non-compliant with the current checklist this sweep, and the oldest yet**: `characters/長 (char).md` carries `date-last-perfect: 2026-03-01` — informal prose in place of standard SKIP/Stroke/MC/Levels bullets, no `## Words`/`## Derived Characters` headings, and `vietnamese` stored as a single malformed comma-separated string rather than a list. Flagged for the character sweep. **Content added to the word page**: `pos: 性詞`; `japanese: ちょう` (the character's only on'yomi); `vietnamese: trường`, picked out of the malformed 5-item string as the reading matching this word's own "long" gloss (trường kỳ, trường thọ) — the string's other members split across the elder/chief sense (trưởng) and an unrelated verb (trườn, "to crawl"); `kwin: true` confirmed.

Built `## Notes` from scratch: the 象形 old-man-with-long-hair etymology (shared top component with [[老]]/[[考]]), the long/elder dual-sense split visible in both the compound family and the Japanese native readings (おさ "chief" vs. なが-い "long"). **Real homophone group found**: `注音 ㄐㄚㄫ`/`장` is also carried by two other existing standalone words, [[杖]] ("staff, stick") and [[章]] ("chapter," whose own page had no `## Notes` section at all — added the minimal callout there without a full rebuild, matching this sweep's anticipatory-touch convention). Added the full mutual callout across all three pages. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 青 (live-computed candidate — `門`/`隶`/`隹` already noted; `雨` was already perfected; `青.md` is the next real, unperfected word file in Kangxi radical-stroke order after `長` — most fields already filled including a stray duplicate `品詞:`, missing `>[!tip]` banner, bare `## Notes` heading, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 524 — [[words/青|青]]

Worked from the alphabetical worklist. `characters/青 (char).md` was already fully perfected. **Content added**: `pos: 性詞` (`品詞:` duplicate dropped); missing `>[!tip]` banner added; `japanese: せい`/`vietnamese: thanh`/`kwin: true` normalized/confirmed correct.

Built `## Notes` from scratch: the 形声 etymology (phonetic 生, originally 丹 "cinnabar" beneath it), the classical CJKV blue/green color-boundary looseness this character is famous for (青色 glossed "blue-green; cyan; grue" in one breath, 青蛙 "frog" sitting in the same Words list, paralleling Japanese blue traffic lights), the youth/immaturity figurative extension (青年, paralleling English "green" for inexperience), and the large purely-phonetic descendant family (請/晴/情/清/猜/睛/精/錆/靖/鯖) with no unifying semantic thread, comparable in scale to [[老]]/[[米]]'s own families earlier in this sweep.

**Real homophone found**: `注音 ㄑㄝㄫ`/`청` is also carried by [[聴]] ("listen"), an existing but unperfected standalone word. Added the full callout to `青` and a matching anticipatory one to `聴`. **Incidental fix**: `聴`'s own `korean` field was blank — filled with 청, matching its character page. Checked 鯖/錆/庁, the remaining `ㄑㄝㄫ`-reading characters — none has a self-pointing `stand_in`. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 頁 (live-computed candidate — `非` was already perfected; `面`/`革`/`韋`/`韭`/`音` have no standalone word file at their radical slot; `頁.md` is the next real, unperfected word file in Kangxi radical-stroke order after `青` — most fields already filled including a stray duplicate `品詞:`, blank `vietnamese:`, bare `## Notes` heading, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 525 — [[words/頁|頁]]

Worked from the alphabetical worklist. **An eighth character page found genuinely unperfected this sweep**: `characters/頁 (char).md` has blank `pos`, `## Notes` holding only two dangling CC-initial/final wikilinks, no `date-last-perfect` — flagged for the character sweep. **Content added**: `pos: 名詞`; `vietnamese: hiệt` (matching this word's own gloss — over the character's other stored reading hệt, actually the unrelated Vietnamese word for "identical, exact"); `japanese: けつ`/`kwin: false` already present, confirmed correct; `品詞:` duplicate dropped.

Built `## Notes` from scratch: the 象形 "head" etymology (頁's older sense, still preserved in its role as the semantic radical for 頭/顔/額 and in its own `korean_native` gloss "머리"), the semantic narrowing to "page" in modern usage, and the character's unusually wide three-on'yomi set (けつ/よう/rare けち) now confined mostly to technical compounds like 頁岩 "shale" since everyday Japanese uses the loanword ページ instead. No homophones (注音 ㄏㄝㄊ shared only by [[歇]], whose own `stand_in` is 間歇). Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 飛 (live-computed candidate — `風` was already perfected; `頁.md` just closed the gap before it; `飛.md` is the next real, unperfected word file in Kangxi radical-stroke order after `頁` — already had most fields filled, but a real `pos` mismatch against its own character page and no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 526 — [[words/飛|飛]]

Continued directly into the next alphabetical item in the same turn (a deviation from the usual one-item-per-firing pacing, same as the 斗/日 double back around iteration 492 — returning to strict single-item cycles from the next firing on). `characters/飛 (char).md` was already fully perfected. **Real content bug found and fixed**: the word's own `pos` was `動詞`, but the character's own `pos` is `事詞` — per this sweep's established convention that a stand-in word always mirrors its own character's `pos` exactly (already confirmed on [[走]]/[[行]]), corrected to `事詞`. **Incidental fix**: both this word's page and `characters/飛 (char).md` itself were missing the `>[!tip]` disambiguation banner entirely — added to both.

Built `## Notes` from scratch: the 象形 bird-in-flight etymology (one of this vault's `hapax`-tagged characters, not anchoring a wide phonetic family the way most characters in this sweep do), the aspectual short-flight/prolonged-flight distinction between bare 飛 and [[飛行]] already documented on the character page, and 飛語/流言飛語's figurative extension to fast-spreading rumor. Flagged the character's `japanese_native: ø` as a likely real gap (と(ぶ) is the ordinary native verb), same pattern as [[手]]/[[玉]] earlier in this sweep. No homophones (注音 ㄈㄝㄧ unique among both words and characters). Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 骨 (live-computed candidate — `馬` was already perfected; `骨.md` is the next real, unperfected word file in Kangxi radical-stroke order after `飛` — most fields already filled including a stray duplicate `品詞:`, bare `## Notes` heading, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 527 — [[words/骨|骨]]

Worked from the alphabetical worklist. `characters/骨 (char).md` was already fully perfected content-wise. **Real `kwin` bug found and fixed on both pages**: both this word's own page and `characters/骨 (char).md` were stamped `kwin: true`, but Dan'a'yo `諺文: 곧` (GOD) and Sino-Korean `골` (GOL) are different Unicode syllables — corrected both to `kwin: false`, the same MC-checked-coda-vs-Korean-liquid pattern already documented on [[月]]/[[越]]/[[舌]] earlier in this sweep (a fifth instance now). **Content added**: `pos: 名詞` (`品詞:` duplicate dropped); `japanese: こつ`/`vietnamese: cốt` normalized to scalars (already correct — cốt picked over the character's noisier remaining variants cút/cọt/gút, which show no independent life for this sense).

Built `## Notes` from scratch: the 形声 etymology (semantic 肉 added to disambiguate the original bone-pictograph phonetic 冎, an unlinkable component also behind [[咼]]/[[拐]]), the systematic anatomical compound family naming individual bones directly, and 骨肉相連 directly paralleling the Biblical "bone of my bone" idiom already noted on [[肉]]. No homophones (注音 ㄍㄛㄊ unique among both words and characters). Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 高 (live-computed candidate — `高.md` is the next real, unperfected word file in Kangxi radical-stroke order after `骨` — most fields already filled including a stray duplicate `品詞:`, unglossed numbered `## Notes` list, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 528 — [[words/高|高]]

Worked from the alphabetical worklist. `characters/高 (char).md` was already fully perfected. **Content added**: `pos: 性詞` (`品詞:` duplicate dropped); `japanese: こう`/`vietnamese: cao`/`kwin: false` already present, confirmed correct — cao notably remains the ordinary everyday Vietnamese word for "high/tall" itself, unlike most Sino-Vietnamese readings in this sweep that stay confined to a learned register.

Built `## Notes` from scratch: the 象形 tall-building etymology (compared on the character's own page to [[京]], a similar tower pictograph that never became its own Kangxi radical), the social/educational-elevation compound family (高人, 崇高, 高等/高校/高考), and 高's coincidental-transliteration role in several place/ethnonym names (高盧 Gaul, 高句麗/高麗 Goguryeo/Goryeo, 高綿 Cambodia). Flagged the character's `japanese_native: ø` as a likely real gap (たかい is extremely common), the same pattern as [[手]]/[[玉]]/[[飛]] earlier in this sweep.

**Real homophone found, a genuine phonetic family**: `注音 ㄍㄚㄨ`/`갓` is also carried by [[稿]] ("manuscript, rough draft") — already listed among 高's own Derived Characters, a direct phonetic descendant. Added the full callout to `高` and a matching anticipatory one to `稿`. Checked 膏/皐/告/藁/縞/睾, the remaining characters sharing this reading — none has a self-pointing `stand_in`. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 鹿 (live-computed candidate — `髟`/`鬥`/`鬯`/`鬲`/`鬼` have no standalone word file at their radical slot; `魚`/`鳥` were already perfected; `鹿.md` is the next real, unperfected word file in Kangxi radical-stroke order after `高` — already carries a `>[!warning] Homophones` callout to [[緑]], but literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 529 — [[words/鹿|鹿]]

Worked from the alphabetical worklist. **A ninth character page found genuinely unperfected this sweep**: `characters/鹿 (char).md` has blank `pos`, `## Notes` holding only two dangling CC-initial/final wikilinks, no `date-last-perfect` — flagged for the character sweep. **Real content bug found and fixed**: the word's own `korean` field was 녹, the South Korean 두음법칙-shifted form — corrected to 록, matching the character's own already-correct field and this vault's standing North Korean rule, the same bug class already found on [[老]] earlier in this sweep. Once corrected, `kwin: true` follows genuinely (록 matches exactly). **Content added**: `pos: 名詞`; `japanese: ろく` (the character's own native reading か is a bound compound-only form, not the free-standing everyday word for deer, which is しか); `vietnamese: lộc` (also noted as carrying a distinct figurative "fortune, blessing" sense in Vietnamese, an extension not paralleled elsewhere in the CJKV readings).
...
### 2026-08-03, iteration 531 — [[words/鼎|鼎]]

Worked from the alphabetical worklist, closing out the [[呈]]/鼎 homophone pair anticipated back on 2026-07-27. **A tenth character page found genuinely unperfected this sweep**: `characters/鼎 (char).md` has blank `pos`, `## Notes` holding only two dangling CC-initial/final wikilinks, no `date-last-perfect` — flagged for the character sweep. **Content added**: `pos: 名詞`; `japanese: てい` (the character's only on'yomi, rare in ordinary vocabulary); `vietnamese: đỉnh` — picked from a noisy 4-item stored set, and notably the same tripod-to-prominence figurative shift found in this vault's own chengyu 春秋鼎盛 is independently echoed in Vietnamese, where đỉnh is the ordinary word for "peak, summit"; `kwin: false` (Dan'a'yo 딩 vs Sino-Korean 정 diverge in the vowel).

Built `## Notes` from scratch: the 象形 three-legged ritual cauldron etymology and its symbolic weight as a marker of state authority (九鼎), the vessel-to-prominence figurative extension, and てい/かなえ. Added the reciprocal homophone callout to `鼎` and updated `呈`'s own stale forward-reference sentence now that this word has come up. Checked 徴/程, the remaining characters sharing this reading — neither has a self-pointing `stand_in`. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 鼓 (live-computed candidate — `鼓.md` is the next real, unperfected word file in Kangxi radical-stroke order after `鼎` — literal `vietnamese: null`, bare `# Notes` placeholder, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 532 — [[words/鼓|鼓]]

Worked from the alphabetical worklist. **An eleventh character page found genuinely unperfected this sweep**: `characters/鼓 (char).md` has blank `pos`, `## Notes` holding only two dangling CC-initial/final wikilinks plus two loose compound bullets, no `date-last-perfect` — flagged for the character sweep. **Content added**: `pos: 名詞`; `japanese: こ` (the character's only on'yomi, covering the broader "drum" sense — narrower than the native つづみ, which names a specific traditional hand-drum, the reverse of this sweep's usual native-is-broader pattern); `vietnamese: cổ` (the character's only stored reading); `kwin: true` confirmed.

Built `## Notes` from scratch: the 会意 etymology (drum-on-stand + 支 "to strike"), explaining why the character covers both the noun "drum" and the verb "to play/drum" in one word, and 鼓舞's "drum-to-encouragement" figurative extension, paralleling English "to drum up support." **Real homophone group found**: `注音 ㄍㄛ`/`고` is also carried by two other existing standalone words, [[股]] ("crotch, groin, fork") and [[錮]] ("obstinate"). Added the full mutual callout across all three pages. Checked 孤/顧/故/固/姑/古, the remaining characters sharing this reading — none has a self-pointing `stand_in`. Stamped `date-last-perfect: 2026-08-03`.

Next (alphabetical): 鼻 (live-computed candidate — `鼠` has no standalone word file at its radical slot; `鼻.md` is the next real, unperfected word file in Kangxi radical-stroke order after `鼓` — most fields already filled including a stray duplicate `品詞:`, bare `## Notes` heading, no `date-last-perfect`). Next (blank-key backlog): 居里金 — both lists remain open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素).

### 2026-08-03, iteration 533 — [[words/鼻|鼻]]

Worked from the alphabetical worklist. `characters/鼻 (char).md` was already fully perfected. **Content added**: `pos: 名詞` (`品詞:` duplicate dropped); `japanese: び`/`vietnamese: tị`/`kwin: true` already present, confirmed correct.

Built `## Notes` from scratch: the 形声 etymology — semantic 自 (originally "nose," before being repurposed to "self," per [[自]]'s own page — 鼻 preserves the meaning its own semantic component lost) + phonetic 畀 (no vault page) — and 阿鼻/阿鼻叫喚 as a purely phonetic Sanskrit loan (Avīci) riding on this syllable. No homophones (注音 ㄅㄧ shared only by characters — 碑, 痺, 脾, 琵, 婢, 皮 — none with a self-pointing `stand_in`). Stamped `date-last-perfect: 2026-08-03`.

**Milestone: the radical-order alphabetical worklist is now exhausted.** Checked the five remaining Kangxi radicals after 鼻 (齊, 齒, 龍, 龜, 龠) — 龍 was already perfected earlier in this sweep, and none of the other four ever had a standalone word file to begin with. Every single-character stand-in word reachable by walking all 214 Kangxi radicals in ascending order has now either been perfected in this sweep or was already done before it started (starting point was [[女]], radical 038, back around iteration 486). Going forward, this thread has no more live candidates — the sweep's only remaining live queue is the blank-key backlog.

Next (blank-key backlog): 居里金 — still open (山岡, 唉, 蘿蔔 remain open and skipped; element-series queue after 居里金: 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素). This becomes the sweep's sole active queue until a new ordering scheme is chosen or the backlog itself is re-scoped.

### 2026-08-03, iteration 534 — [[words/居里金|居里金]]

Worked from the blank-key backlog, the first iteration since the alphabetical worklist's exhaustion last cycle. Curium (96) — the entry that opens this cycle's continuation of the avoid-the-real-element-character series. **Content added**: `mandarin: jūlǐjīn`/`cantonese: geoi1 lei5 gam1`, the plain compositional concatenation of 居/里/金's own readings (both were blank); `vietnamese: Curi`, the nativized loanword continuing this series' "-i" ending pattern already seen on [[日本素]]'s Nihoni; added the `>[!tip]` banner (missing entirely). Confirmed via web search that curium's real Chinese chemistry character is 锔 (jú), unrelated to any of this word's three constituent characters — the same avoided-real-character pattern as every other entry in this series.

`羅馬字`/`諺文`/`注音` (giligim/기리김/ㄍㄧㄌㄧㄍㄧㄇ) cross-checked against 居/里/金's own fields — clean concatenation. `kwin: false` confirmed via the same AND-rule exception already documented on [[丹金]]: the authoritative plain-string comparison of 諺文 vs `korean` is what governs, not the constituent characters' own `kwin` values. Expanded the existing one-line Note into full prose: 居里's status as the standard two-syllable Mandarin transliteration of "Curie" (already complete, not truncated, unlike some other scientist-named elements in this series), and Japanese/Korean's shared pattern of borrowing the IUPAC name wholesale rather than deriving it compositionally. Stamped `date-last-perfect: 2026-08-03`.

Next (blank-key backlog): 愛因金 (live-computed — the element-series queue's next entry after 居里金; already has most fields filled in the same style as 居里金 before this iteration — blank `mandarin`/`cantonese`/`vietnamese`, no `>[!tip]` banner, thin one-line Notes, no `date-last-perfect`). Queue after 愛因金: 柏克金, 美洲金, 羅倫金, 西博金, 火紅素 (山岡, 唉, 蘿蔔 remain separately open and skipped).

### 2026-08-03, iteration 535 — [[words/愛因金|愛因金]]

Worked from the blank-key backlog. Einsteinium (99). **Content added**: `mandarin: àiyīnjīn`/`cantonese: oi3 jan1 gam1`, the plain compositional concatenation of 愛/因/金's own readings; `vietnamese: Einsteini`, confirmed via web search (Vietnamese sources use "Einsteini"/"einsteni," continuing this series' "-i" ending pattern already seen on [[日本素]]/[[居里金]]); added the missing `>[!tip]` banner. Confirmed the real Chinese chemistry character for einsteinium is 锿 (āi), unrelated to any of this word's three constituent characters — the same avoided-real-character pattern as the rest of this series.

Expanded the existing one-line Note into full prose: 愛因 truncates only the first two syllables of the full transliteration 愛因斯坦, unlike [[居里金]] (Curie), whose transliteration was already short enough to use whole — a real contrast between two adjacent entries in the same series worth noting explicitly. `kwin: false` confirmed via the same AND-rule exception already documented on [[丹金]]/[[居里金]] (諺文 애인김 vs `korean` 아인슈타이늄 plainly differ). Stamped `date-last-perfect: 2026-08-03`.

Next (blank-key backlog): 柏克金 (live-computed — the element-series queue's next entry after 愛因金; same pre-perfection state as 愛因金 before this iteration — blank `mandarin`/`cantonese`/`vietnamese`, no `>[!tip]` banner, thin one-line Notes, no `date-last-perfect`). Queue after 柏克金: 美洲金, 羅倫金, 西博金, 火紅素 (山岡, 唉, 蘿蔔 remain separately open and skipped).

### 2026-08-03, iteration 536 — [[words/柏克金|柏克金]]

Worked from the blank-key backlog. Berkelium (97). **Content added**: `mandarin: bókèjīn`/`cantonese: paak3 hak1 gam1` (compositional concatenation — noting 柏 in this proper-noun transliteration role reads bó, the same reading it carries in 柏林 "Berlin," an unrelated place-name coincidence rather than any shared etymology); `vietnamese: Beckeli`, confirmed via web search, continuing the series' "-i" pattern; added the missing `>[!tip]` banner. Confirmed the real Chinese chemistry character for berkelium is 锫 (péi), unrelated to any of this word's three constituent characters. `kwin: false` confirmed via the same AND-rule exception documented across this series. Stamped `date-last-perfect: 2026-08-03`.

### 2026-08-03, iteration 537 — [[words/美洲金|美洲金]]

Continued directly into the next blank-key-backlog item in the same turn (a deviation from one-item-per-firing pacing, same as prior doubles in this sweep — returning to strict single-item cycles next cycle). Americium (95). **A different naming logic from the previous several entries**: unlike [[愛因金]]/[[柏克金]] (both scientist-surname transliterations), 美洲金 calques the etymology directly — "named after the Americas," by analogy with europium — the same place-name-calque logic already used for [[日本素]] (Nihonium). **Content added**: `mandarin: měizhōujīn`/`cantonese: mei5 zau1 gam1`; `vietnamese: Americi`, confirmed via web search — notably the "-i" nativized-loanword pattern holds even though this name is a calque rather than a personal-name transliteration, since Vietnamese/Japanese/Korean all borrow the international scientific name wholesale regardless of underlying etymology; added the missing `>[!tip]` banner. Confirmed the real Chinese chemistry character for americium is 镅 (méi). `kwin: false` confirmed via the same AND-rule exception. Stamped `date-last-perfect: 2026-08-03`.

Next (blank-key backlog): 羅倫金 (live-computed — the element-series queue's next entry after 美洲金; same pre-perfection state as the others before this iteration — blank `mandarin`/`cantonese`/`vietnamese`, no `>[!tip]` banner, thin one-line Notes, no `date-last-perfect`). Queue after 羅倫金: 西博金, 火紅素 (山岡, 唉, 蘿蔔 remain separately open and skipped).

### 2026-08-03, iteration 538 — [[words/羅倫金|羅倫金]]

Worked from the blank-key backlog, one item this cycle. Lawrencium (103). **Content added**: `mandarin: luólúnjīn`/`cantonese: lo4 leon4 gam1` (compositional concatenation); `vietnamese: Lawrenci` (also attested as Lorenxi/Lorenci), confirmed via web search, continuing the series' "-i" pattern; added the missing `>[!tip]` banner. Confirmed the real Chinese chemistry character for lawrencium is 铹 (láo), unrelated to any of this word's three constituent characters.

Expanded the existing Note: 羅倫 truncates 勞倫斯/羅倫斯 to two syllables, the same pattern as [[愛因金]]/[[柏克金]] — all three named for Berkeley-connected physicists (Einstein, Berkeley the place, and now Lawrence, inventor of the cyclotron and founder of the Lawrence Berkeley lab where this element itself was discovered). `kwin: false` confirmed via the same AND-rule exception documented across this series. Stamped `date-last-perfect: 2026-08-03`.

Next (blank-key backlog): 西博金 (live-computed — the element-series queue's next entry after 羅倫金; same pre-perfection state as the others — blank `mandarin`/`cantonese`/`vietnamese`, no `>[!tip]` banner, thin one-line Notes, no `date-last-perfect`). Queue after 西博金: 火紅素 (last of the currently-known element-series entries; 山岡, 唉, 蘿蔔 remain separately open and skipped).

### 2026-08-03, iteration 539 — [[words/西博金|西博金]]

Worked from the blank-key backlog. Seaborgium (106), named for chemist Glenn T. Seaborg — a fitting close to this run of actinide/transactinide entries, since Seaborg himself pioneered the modern periodic-table placement conventions this whole naming sub-series has been patterned on, and this element was named for him while he was still living, an exception IUPAC made to its usual rule. **Content added**: `mandarin: xībójīn`/`cantonese: sai1 bok3 gam1` (compositional concatenation); `vietnamese: Seaborgi`, confirmed via web search, continuing the series' "-i" pattern; added the missing `>[!tip]` banner. The real Chinese chemistry name for seaborgium is built on a dedicated single glyph (釒+phonetic 喜, read xǐ, sometimes displayed as separate 金喜 when the combined glyph doesn't render) — noted with appropriate hedging since the exact glyph is a rare Unicode Extension B character not independently re-verified beyond search results. `kwin: false` confirmed via the same AND-rule exception documented across this series. Stamped `date-last-perfect: 2026-08-03`.

### 2026-08-03, iteration 540 — [[words/火紅素|火紅素]]

Continued directly into the last known element-series entry in the same turn (a deviation from one-item-per-firing pacing, same as prior doubles in this sweep). Strontium (38) — a genuinely different case from every other entry in this series: a **descriptive coinage**, not a transliteration, naming the element after its signature crimson flame-test color (火紅, "fire-red") rather than the real English name's geographic origin (Strontian, Scotland). The existing page already had rich content but used non-canonical numbered `### 1/2/3` headers instead of this vault's standard flowing-prose Notes — rewrote into plain paragraphs while preserving the substance (the flame-test property, the deliberate contrast with [[茜素]]'s botanical-red naming logic, the phenomenological-vs-toponymic etymological framing, and 火 alone as the periodic-table symbol).

**Content added**: `mandarin: huǒhóngsù`/`cantonese: fo2 hung4 sou3` (compositional concatenation); `japanese: ストロンチウム`/`korean: 스트론튬`, both the IUPAC loanword per this series' established convention — **caught and corrected my own initial mistake here**: I first set `korean` to a compositional concatenation matching the 諺文 field (화홍소), but every other entry in this series uses the real Korean IUPAC loanword for this field instead (confirmed by checking [[日本素]]/[[石素]]), so corrected to 스트론튬 and `kwin` to `false` accordingly. `vietnamese: Stronti`, confirmed via web search. Added the `>[!tip]` banner and `pos: 固有名詞` (`品詞:` duplicate dropped). Stamped `date-last-perfect: 2026-08-03`.

**The element-series queue is now fully cleared.** All entries flagged across this backlog thread (丹金, 居里金, 愛因金, 柏克金, 美洲金, 羅倫金, 西博金, 火紅素, plus earlier-cleared entries) are resolved. The blank-key backlog's only remaining open items are three older, separately-flagged pages: 山岡, 唉, 蘿蔔.

Next (blank-key backlog): 山岡 (live-computed — checked why this has stayed open across many prior iterations: its `korean` field holds a native Korean *gloss* — "언덕, 구릉" ("hill, mound") — rather than an actual Sino-Korean pronunciation, a real data-format bug distinct from a simple blank field; character pages for both constituents, 山 and 岡, do exist, so the block isn't a missing-character situation like some earlier entries in this sweep. Needs a real Sino-Korean reading researched before this can be fixed, not just a mechanical concatenation. No `date-last-perfect`, no `kwin` field either).

### 2026-08-03, iteration 541 — [[words/山岡|山岡]]

Worked from the blank-key backlog. **Real content bug found and fixed, explaining why this page had stayed open for so long**: `korean` held a native Korean gloss ("언덕, 구릉," "hill, mound") rather than an actual pronunciation. The real Sino-Korean reading is the plain compositional concatenation 산강 (山's 산 + 岡's 강) — which coincidentally matches the word's own already-correct Dan'a'yo 諺文 field exactly, since both constituent characters independently carry `kwin: true`. Added `kwin: true` accordingly. `vietnamese: sơn cương` was missing entirely (not blank, absent) — filled compositionally from 山's sơn and 岡's own sole stored reading, cương. `pos`/mandarin/cantonese/japanese were already present and correct.

Built `## Notes` from scratch: this word is the `stand_in` legitimizer for [[岡]] (per the character's own `stand_in: 山岡`), and 山岡 survives most vividly in Japanese as やまおか (Yamaoka), a real surname/place name using both characters' native readings rather than an on'yomi concatenation — already correctly stored in the word's own frontmatter before this iteration. Noted that 岡 is a comparatively minor character added to this vault specifically for its role in Japanese prefecture names (per its own character-page note), so this compound's real cross-linguistic life leans Japanese rather than being evenly spread across the whole CJKV sphere. No homophones (注音 ㄙㄚㄋㄍㄚㄫ unique). Stamped `date-last-perfect: 2026-08-03`.

### 2026-08-03, iteration 542 — [[words/唉|唉]]

Continued directly into the next blank-key-backlog item in the same turn (a deviation from one-item-per-firing pacing). **Investigated, but deliberately left unresolved, the reason this page had stayed open**: both this word's page and `characters/唉 (char).md` carry a note — "consider blending in with the mouth-less version" — referring to [[矣]], the leftover character when 唉's own [[口]] radical is removed and named as its phonetic component. Checked whether this warrants a merge like the [[狡]]/[[滑]] (猾) precedent: 矣 has its own fully independent character page with a Dan'a'yo reading (⼔/워) that has drifted completely away from 唉's own (ㄚㄧ/애) despite the phonetic-derivation relationship, and neither page's `aliases:` field lists the other. Per the standing rule that this exact kind of merge-or-keep-separate call belongs to the user, not to whoever is doing routine upkeep, left the question open exactly as before — only completed the surrounding fields.

**Content added**: `pos: 感詞`; `japanese: あい` (compositionally, matching the character's on'yomi AI among its unusually wide four-reading set KAI/KI/AI/I); `vietnamese: ai`, confirmed via web search against Hán Việt dictionaries specifically for this "alas, lament" sense — already the first-listed candidate among the character's own noisy 8-item stored set, so no contamination to sort through this time; `kwin: true` (諺文/korean both 애). **Real homophone group found**: `注音 ㄚㄧ`/`애` is also carried by two other existing standalone words, [[崖]] ("cliff") and [[愛]] ("love") — added the full mutual callout across all three pages, the widest-reaching group this sweep has found given how common 愛 is. Checked 碍/埃/艾/挨/曖/哀/靄, the remaining characters sharing this reading — none has a self-pointing `stand_in`. Stamped `date-last-perfect: 2026-08-03`.

Next (blank-key backlog): 蘿蔔 (live-computed — the last of the three previously-stuck blank-key-backlog items; already has most fields filled (mandarin/cantonese/japanese/korean/vietnamese all present, `kwin: true`), but no `date-last-perfect` and worth checking why it was skipped before assuming it's simply complete).

### 2026-08-03, iteration 543 — [[words/蘿蔔|蘿蔔]]

Worked from the blank-key backlog, the last of the three previously-stuck items. **Real content bug found and fixed, explaining why this page had stayed open**: `羅馬字`/`諺文`/`注音` used a second-syllable reading (bok/복/ㄅㄛㄎ) that didn't match `characters/蔔.md`'s own stored Dan'a'yo reading (bug/북/ㄅㄨㄎ) — both are real syllable pages in this vault, so this wasn't a missing-syllable gap, just a wrong-syllable citation on the word page. Corrected to `labug`/`라북`/`ㄌㄚㄅㄨㄎ`. This flips `kwin` from `true` to `false`: 蔔 on its own already carries `kwin: false` (its Dan'a'yo 북 diverges from Sino-Korean 복), and once the word's Dan'a'yo reading was corrected to match 蔔's real value, it no longer matches the word's own `korean` field (라복, a genuinely attested Sino-Korean concatenation, left unchanged) — so the compound's `kwin` must be `false` too.

Confirmed `vietnamese: la bặc` is a real attested Hán Việt compound reading via web search (alongside the related hồng la bặc, "carrot") rather than a naive concatenation of 蔔's own stored bốc, so no change needed there. Built `## Notes` from scratch: this word as the `stand_in` legitimizer for [[蔔]], the classical variant spellings 蘿菔/蘆菔, and Japanese らふく as an on'yomi reading that diverges more than usual from the everyday native word だいこん (daikon). Stamped `date-last-perfect: 2026-08-03`.

**Milestone: the blank-key backlog's three previously-stuck items (山岡, 唉, 蘿蔔) are now all resolved, alongside the earlier-cleared element series.** Both of this sweep's tracked queues — the radical-order alphabetical worklist (exhausted at iteration 533) and the blank-key backlog (exhausted now) — are empty. Per the project history's own account (`AIOS/projects.md`), a much longer tail of older batch-created word pages with isolated blank `vietnamese:` keys still exists beyond what this sweep has been tracking explicitly — re-scoped below.

**Re-scoping the backlog**: a first mechanical scan (grep for a bare `vietnamese:` line) returned 2439 hits, but this overcounted badly — it can't distinguish a truly empty field from a YAML *list*-form field whose value sits on the following lines (`vietnamese:\n  - foo`), which reads as a bare `vietnamese:` line too. A corrected Python scan (checking whether the line immediately after `vietnamese:` actually starts with a list-item dash) narrowed this to **2047 genuinely blank-`vietnamese`, never-`date-last-perfect`-stamped word files** — still a very long tail, confirming the scale the project history already described. New convention going forward: alphabetical by filename through this corrected list, mirroring how the very first stretch of this sweep (iterations 1–20ish, per this file's own earlier history) worked the original HSK-1 pool.

### 2026-08-03, iteration 544 — [[words/好|好]]

First iteration of the newly re-scoped tail. `characters/好 (char).md` was already fully perfected. **Note on how this candidate was found**: 好 came up as a false positive in the flawed first-pass grep (its `vietnamese:` field already held real content, hiếu/hảo, just in list form) — but it turned out to be a legitimately unperfected page anyway (no `date-last-perfect`, blank `japanese`, stray duplicate `品詞:`), so it was worked directly rather than discarded. **Content added**: `pos: 性詞` (`品詞:` duplicate dropped); `japanese: こう` (the character's only on'yomi); `vietnamese: hiếu, hảo` kept as a genuine two-way split (hảo = general "good," hiếu = filial piety specifically — a real semantic divergence traced to the character's own mother-child etymology, not contamination); `kwin: false` confirmed.

Built `## Notes` from scratch: the 会意 etymology (女+子, "good for a woman to have a child"), the adjective/verb duality already present in Mandarin's own tone split (hǎo/hào) mirrored by two distinct Japanese native readings (この(む)/い-い), and the hảo/hiếu Vietnamese split. **Real homophone found**: `注音 ㄏㄚㄨ`/`핫` is also carried by [[毫]] ("fine hair, milli-"), an existing but unperfected standalone word — added the full callout to both pages. Checked 昊/鎬/豪/耗/皓/浩/号, the remaining characters sharing this reading — none has a self-pointing `stand_in`. Stamped `date-last-perfect: 2026-08-03`.

Next (re-scoped tail, alphabetical-by-filename): 一体 (live-computed — first entry in the corrected 2047-file scan; stray unglossed placeholder body text "J meaning is gone," blank `vietnamese:`/`hsk_level:`/`swadesh:`/`aliases:` fields left in as empty keys rather than omitted, duplicate `品詞:`, no `date-last-perfect`).

### 2026-08-03, iteration 545 — [[words/一体|一体]]

Worked from the re-scoped tail. Removed the stray placeholder body text ("J meaning is gone") that sat before the `meta-bind-embed` block. **Content added**: `vietnamese: nhất thể`, a real attested Hán Việt compound (nhất thể hóa, "unification") — researched independently since both `一 (char).md` and `体.md` themselves have blank `vietnamese` fields, a genuine gap on both character pages flagged here but left for the character sweep rather than fixed; `pos: 名詞`; blank `hsk_level:`/`swadesh:` dropped per the blank-optional-field rule, `品詞:` duplicate dropped, `aliases: []` (empty list) normalized to a real one-item list (一體). `japanese: いったい`/`korean: 일체`/`kwin: false` were already present and confirmed correct.

Built `## Notes` from scratch: the "one body" etymology underlying both the concrete "organic whole" and abstract "unity" senses, and Japanese's distinctive third sense as an emphatic interrogative intensifier (いったい何, "what on earth") with no real Mandarin/Cantonese parallel — the sokuon gemination いち→いっ- before た- documented as the regular phonological process it is. No homophones (注音 ㄧㄊㄊㄝㄧ unique). Stamped `date-last-perfect: 2026-08-03`.

Next (re-scoped tail, alphabetical-by-filename): 一定 (live-computed — next entry after 一体 in the corrected scan; blank `pos`, blank `vietnamese:`/`hsk_level:`/`swadesh:`, `aliases: []` empty list, no `date-last-perfect`).

### 2026-08-03, iteration 546 — [[words/一定|一定]]

Worked from the re-scoped tail. **Real `kwin` bug found and fixed at its root cause, with wider implications flagged**: both this word and `characters/一 (char).md` itself were stamped `kwin: true`, but Dan'a'yo `諺文: 읻` (ID) and Sino-Korean `korean: 일` (IL) are different Unicode syllables (一 retains the MC checked coda that Sino-Korean regularized to -l, the same pattern as [[月]]/[[越]]/[[舌]]/[[骨]]). Corrected `kwin` to `false` on both — and since 一 is one of the most common characters in the whole corpus, **any other 一-initial compound that simply copied 一's own stored (wrong) `kwin` value rather than independently checking may carry the same latent error**, worth re-checking whenever a future iteration touches one. (Caught my own mistake mid-edit here too: I first wrote a Notes paragraph asserting `kwin: true` without verifying byte-for-byte, then caught and corrected it before finishing the iteration.)

**Content added**: `pos: 性詞`; `vietnamese: nhất định`, a very well-attested Hán Việt compound (confirmed via web search) preserving the same adjective ("a certain/given degree") vs. adverb ("definitely, without fail") split found in the Mandarin source, explicitly distinguished in Chinese pedagogy from the near-synonym 肯定; blank `hsk_level:`/`swadesh:` dropped, `aliases: []` normalized. Built `## Notes` covering this dual sense, the sokuon いってい (paralleling [[一体]]'s いったい), and Japanese's narrower "fixed, set" sense without the emphatic-certainty adverbial use (which Japanese instead expresses via きっと/必ず). No homophones (注音 ㄧㄊㄐㄝㄫ unique). Stamped `date-last-perfect: 2026-08-03`.

Next (re-scoped tail, alphabetical-by-filename): 一斉 (live-computed — next entry after 一定 in the corrected scan; blank `pos`/`korean:`/`vietnamese:`/`hsk_level:`, `aliases: []` empty list while `swadesh:` instead holds what looks like misplaced alias content (`一齊, 一齐`) — a field-swap bug worth investigating, not just a blank fill — no `date-last-perfect`).

### 2026-08-03, iteration 547 — [[words/一斉|一斉]]

Worked from the re-scoped tail. **Confirmed and fixed the flagged field-swap bug**: 一齊/一齐 (traditional/simplified variants) had been stored in `swadesh:` instead of `aliases:` — moved to `aliases:` as a proper list, `swadesh:` cleared. **Content added**: `pos: 副詞`; `korean: 일제` (compositional concatenation, genuinely attested); `vietnamese: nhất tề`, confirmed via web search. This word is the `stand_in` legitimizer for [[斉]] (per the character's own `stand_in: 一斉`).

**Another `kwin` bug from the same 一-initial root cause found on [[一定]] last iteration**: newly-filled `korean: 일제` diverges from the existing Dan'a'yo `諺文: 읻제` in the first syllable's coda (읻 vs 일) — set `kwin: false` rather than assuming `true`. Built `## Notes` covering the "one and aligned" etymology, いっせい/一斉に as Japanese's ordinary adverbial form, and Vietnamese nhất tề's matching "together, in unison" sense. No homophones (注音 ㄧㄊㄐㄝㄧ unique). Stamped `date-last-perfect: 2026-08-03`.

Next (re-scoped tail, alphabetical-by-filename): 一旦 (live-computed — next entry after 一斉 in the corrected scan; blank `vietnamese:`/`hsk_level:`/`swadesh:`, `aliases: []` empty list, `korean:` holds a reading-plus-appended-gloss mix ("일단, 먼저 한번") rather than a clean pronunciation — another field-hygiene bug to sort out, not just a blank fill — a stray unglossed body line ("once, on one day, in one day") before the `meta-bind-embed` block, no `date-last-perfect`).

### 2026-08-03, iteration 548 — [[words/一旦|一旦]]

Worked from the re-scoped tail. **Confirmed and fixed both flagged bugs**: the stray body line duplicating the `english` gloss (removed, violated meta-bind-embed-first rule); `korean` held a reading-plus-appended-Korean-gloss mix ("일단, 먼저 한번") — trimmed to the clean reading 일단. **Content added**: `vietnamese: nhất đán`, a genuinely attested classical/literary Hán Việt term (confirmed via web search) covering three senses — literal "in a single day" (emphasizing swiftness), conditional "once, as soon as," and modern-Vietnamese "suddenly, unexpectedly." `kwin: false` via the same [[一 (char)]] root-cause pattern already fixed this stretch. Stamped `date-last-perfect: 2026-08-03`.

### 2026-08-03, iteration 549 — [[words/一朝|一朝]]

Continued directly into the next re-scoped-tail item in the same turn (a deviation from one-item-per-firing pacing). **Structural fix**: a stray plain-text "Homophones: [[一兆]] - one trillion" line before `meta-bind-embed` — converted to a proper `>[!warning] Homophones` callout; the reciprocal side on [[一兆]]'s own page had the identical bug, given the same minimal fix (callout format only, not a full perfection pass — 一兆 itself remains open for its own turn). **Content added**: `vietnamese: nhất triêu`, confirmed via web search — 朝 splits into two Vietnamese readings by sense (triêu "morning" vs. triều "royal court/dynasty," the same character-splits-by-meaning pattern already seen on [[行]]), triêu picked to match this word's "short time" gloss. `kwin: false`, the same [[一 (char)]] root-cause pattern, now confirmed across four consecutive 一-initial compounds in this stretch ([[一定]]/[[一斉]]/[[一旦]]/一朝). Stamped `date-last-perfect: 2026-08-03`.

Next (re-scoped tail, alphabetical-by-filename): live-computed — a fresh scan is running to find the next candidate after 一朝; check the log's own tail on the next firing rather than trusting a stale name here, since the exact next file wasn't confirmed before this entry was written.

### 2026-08-03, iteration 550 — [[words/一溝|一溝]]

Worked from the re-scoped tail (rerun scan confirmed 2042 files remaining, down from 2047 — five recent perfections registered correctly; 一溝 is the alphabetically-first survivor). 一溝 names 10³², one of the ten traditional large-number units (亿/兆/京/垓/秭/穰/溝/澗/正/載) — 溝's everyday "ditch, gutter" sense is unrelated, the character simply repurposed as a place-value name the same way 兆/京 were. **Content added**: `korean: 일구` (compositional concatenation); `pos: 数詞` (`品詞:` duplicate dropped); `kwin: false`, the same [[一 (char)]] root-cause pattern now confirmed on a fifth consecutive 一-initial compound this stretch. `vietnamese` deliberately left omitted — no independently attested Sino-Vietnamese reading for 溝 in this large-number sense turned up in search, and Vietnamese doesn't productively use the classical Chinese large-unit hierarchy beyond the 億/兆 range — documented as a genuine gap rather than guessed. No homophones (注音 ㄧㄊㄍㄛㄨ unique). Stamped `date-last-perfect: 2026-08-03`.

Next (re-scoped tail, alphabetical-by-filename): 一端 (live-computed — next entry after 一溝 in the corrected scan; blank `pos`/`vietnamese:`/`hsk_level:`/`swadesh:`, `aliases: []` empty list, `kwin: false` already present — worth double-checking against the same [[一 (char)]] root-cause pattern rather than assuming it was independently verified).

### 2026-08-03, iteration 551 — [[words/一端|一端]]

Worked from the re-scoped tail. **Content added**: `pos: 名詞`; `vietnamese: nhất đoan`, confirmed via web search — 端's own "beginning, origin, cause" sense (見端/造端) is what this compound draws on, not the more common "end/edge" sense. `kwin: false` double-checked and confirmed already correct.

Built `## Notes` covering the concrete-tip-to-abstract-aspect metaphor, and a genuine cross-linguistic coincidence: this word's Japanese reading いったん is identical to [[一旦]]'s, despite the two being semantically unrelated and written with different second characters (端 vs 旦) — a Japanese-specific homophony that doesn't carry over to any other language in this vault's CJKV set, distinct from this sweep's usual Dan'a'yo-level homophone groups. No homophones at the Dan'a'yo level (注音 ㄧㄊㄉ⺢ㄋ unique). Stamped `date-last-perfect: 2026-08-03`.

Next (re-scoped tail, alphabetical-by-filename): 一角獣 (live-computed — next entry after 一端 in the corrected scan; blank `vietnamese:`/`hsk_level:`/`swadesh:`, stray duplicate `品詞:`, no `date-last-perfect`; already carries real aliases 獨角獸/独角兽/一角馬).

### 2026-08-03, iteration 552 — [[words/一角獣|一角獣]]

Worked from the re-scoped tail. **Real content bug found and fixed**: `cantonese` was stored as `jat1 gok3 sau3`, a reading of this word's own literal 一角獸 form — but web search confirmed the actually-used Cantonese term for unicorn is 獨角獸 (duk6 gok3 sau3), already correctly used for `mandarin` and already listed among the word's own `aliases`. Corrected `cantonese` to match. Documented, not "fixed away," the real cross-linguistic split this surfaced: Japanese settles on 一角獣/いっかくじゅう (this word's own primary form) while Mandarin/Cantonese both default to the 獨角獸/独角兽 phrasing instead — two different calque choices for the same imported concept, not an error to unify further.
...
### 2026-08-03, iteration 555 — [[words/七千|七千]]

Worked from the re-scoped tail. **Content added**: `cantonese: cat1 cin1`; `vietnamese: thất thiên` (plain compositional Hán Việt, everyday Vietnamese counting natively instead — bảy nghìn/bảy ngàn); `品詞:` duplicate dropped. **Real `kwin` bug found and fixed on two pages**: this word and `characters/七 (char).md` itself were both stamped `kwin: true`, but Dan'a'yo 칟 (CID) and Sino-Korean 칠 (CIL) are different syllables (七 retains the MC checked coda that Sino-Korean regularized to -l) — the same root-cause pattern already fixed on [[一 (char)]], now confirmed on a second common numeral character. Corrected both to `kwin: false`. Also noted なな replacing しち in ななせん, avoiding confusion with いち (one) in spoken numerals.

**Real corpus bug found while checking homophones**: `words/七万.md` ("seventy thousand") turned out to be an accidental full duplicate of this page's content — every field except `characters:` itself described "seven thousand," inheriting 千's reading instead of 万's own. Rewritten from scratch in the same turn.

### 2026-08-03, iteration 556 — [[words/七万|七万]]

Continued directly into fixing the duplicate-content bug found while working on [[七千]] (a deviation from one-item-per-firing pacing, justified since the bug was directly adjacent and left the vault with factually wrong content otherwise). Rebuilt every field with 万's own real readings: `mandarin: qīwàn`/`cantonese: cat1 maan6`/`japanese: ななまん`/`korean: 칠만`/`羅馬字: citmon`/`諺文: 칟몬`/`注音: ㄑㄧㄊㄇㄛㄋ`. `vietnamese: bảy vạn` — notably a fully live, ordinary Vietnamese numeral (vạn = 10,000, used in everyday phrases like vạn sự như ý) rather than a classical-register form, unlike 七千's own thất thiên. `kwin: false`, the same [[七 (char)]] root-cause pattern just fixed on 七千. Stamped `date-last-perfect: 2026-08-03` on both pages.

Next (re-scoped tail, alphabetical-by-filename): 七情 (live-computed — next entry after 七万 in the corrected scan; already has rich existing content — three enumerated lists of the "seven emotions" per Confucian/Buddhist/Traditional-Chinese-Medicine traditions — but blank `pos:`/`korean:`/`vietnamese:`, stray duplicate `品詞:`, no `date-last-perfect`).

### 2026-08-03, iteration 557 — [[words/七情|七情]]
