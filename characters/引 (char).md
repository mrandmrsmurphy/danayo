---
mandarin: yǐn
cantonese: jan5
korean: 인
korean_native: 끌
japanese:
  - IN
japanese_native: ひ
vietnamese:
  - dẫn
  - dận
  - dợn
  - giận
  - giỡn
middle_chinese_initial: j
middle_chinese_final: iɪn
stroke_count: 4
radical: 弓
skip_number: 1-3-1
grade_level: "2"
pos: 動詞
english:
  - pull
  - tug
羅馬字: "'in"
諺文: 인
joyo_level: "2"
hsk_level: "2"
hanmun_edu_level: 中
danayo_id: 2090
mc_id: 569
graphemic_classification: 会意
stand_in: 引
aliases:
注音: ㄧㄋ
date-last-perfect: 2026-07-24
kwin: true
tags:
  - character
boundedness: 100
---
>[!tip] This is a page about the character 引.
>For the word, see [引](words/引.md)
```meta-bind-embed
[[nav/char_info]]
```
## Notes
- [List of 会意](lookup/List%20of%20会意.md): [[Radical 057|弓]] ("bow") + [[Radical 004|丿]] ("indicatory stroke") — oracle-bone and bronze forms depict a slack bow with a short slant stroke marking the place of drawing the bowstring; "to pull, to draw (a bow)."
- [SKIP-1-3-1](lookup/SKIP/SKIP-1/SKIP-1-3-1.md) ([Stroke 04](lookup/Stroke/Stroke%2004.md))
- 569th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 以|j]] + [[Lookup/CC/finals/韻 眞A開|iɪn]] → [ㄧㄋ](syllables/ㄧㄋ.md)
- [Grade 2](lookup/Grade%202.md), [Old HSK 2](lookup/HSK/Old%20HSK%202.md), [Jōyō - Kyōiku](lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[引]]<rt>ㄧㄋ</rt></ruby> "pull; tug" (stand-in for 引)
- <ruby>[[引出]]<rt>ㄧㄋㄑㄨㄊ</rt></ruby> "to lead out; to extract"
- <ruby>[[引禍]]<rt>ㄧㄋㄏ⺢</rt></ruby> "to bring on calamity"
- <ruby>[[牽引]]<rt>ㄎㄝㄋ·ㄧㄋ</rt></ruby> "tow; traction"
- <ruby>[[勾引]]<rt>ㄍㄛㄨㄧㄋ</rt></ruby> "seduce; entice"
- <ruby>[[引入]]<rt>ㄧㄋㄋㄧㄆ</rt></ruby> "lead into"
- <ruby>[[丘引]]<rt>ㄎ⼜·ㄧㄋ</rt></ruby> "earthworm"
- <ruby>[[割引]]<rt>ㄍㄚㄊㄧㄋ</rt></ruby> "discount"

## Chengyu
- <ruby>[[招災引禍]]<rt>ㄑㄛㄨㄐㄚㄧ·ㄧㄋㄏ⺢</rt></ruby> "invite disaster, cause trouble"
- <ruby>[[引出奴家]]<rt>ㄧㄋㄑㄨㄊㄋㄛㄍㄚ</rt></ruby> "I lead you out of slavery"
