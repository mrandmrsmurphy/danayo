---
mandarin: hū
cantonese: fu4
korean: 호
korean_native: 어조사
japanese:
  - KO
japanese_native: か
vietnamese:
  - hồ
  - hô
middle_chinese_initial: ɣ
middle_chinese_final: uo
stroke_count: 5
radical: 丿
skip_number: 4-5-3
grade_level: "2"
pos: 感詞
english:
  - did
  - do
羅馬字: ho
諺文: 호
joyo_level: 日本人名用漢字
hsk_level: "2"
hanmun_edu_level: 中
danayo_id: 2003
mc_id: 69
graphemic_classification: 会意
stand_in: 乎
aliases:
  - 𠂞
  - 𠂠
注音: ㄏㄛ
date-last-perfect: 2026-07-24
tags:
  - character
kwin: true
boundedness: 90
---
>[!tip] This is a page about the character 乎.
>For the word, see [乎](words/乎.md)

```meta-bind-embed
[[nav/char_info]]
```
## Notes
- 会意/形声 (OC \*ɢaː): semantic "wind; air" + phonetic 丂 (OC \*kʰluːʔ, "branch") — the original form of 呼 (OC \*qʰaː, "to call; to cry"), later differentiated from it; a grammatical particle.
- [SKIP-4-5-3](lookup/SKIP/SKIP-4/SKIP-4-5-3.md) ([Stroke 05](lookup/Stroke/Stroke%2005.md))
- 69th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 匣|ɣ]] + [[Lookup/CC/finals/韻 模|uo]] → [ㄏㄛ](syllables/ㄏㄛ.md)
- [Grade 2](lookup/Grade%202.md), [Old HSK 2](lookup/HSK/Old%20HSK%202.md), [Jinmeiyō](lookup/Japanese/Jinmeiyō.md), [Korean MS](lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[乎]]<rt>ㄏㄛ</rt></ruby> "did; do" (stand-in for 乎)
- <ruby>[[不亦V乎]]<rt>ㄅㄛㄊㄜㄎㄏㄛ</rt></ruby> "circumfix for rhetorical questions"

## Chengyu
- <ruby>[[豹斑改乎]]<rt>ㄅ⼘ㄨㄅㄚㄋㄍㄚㄧㄏㄛ</rt></ruby> "can a leopard change his spots?"

## Derived Characters
- <ruby>[[呼 (char)|呼]]<rt>ㄏㄛ</rt></ruby> "to call; shout; exhale"
