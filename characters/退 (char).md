---
mandarin: "tuì"
cantonese: teoi3
korean: "퇴"
korean_native: "물러날"
japanese:
  - TAI
  - TON
japanese_native: しりぞ
vietnamese:
  - thoái
  - thui
  - thói
  - thúi
  - thối
  - thụi
  - thủi
middle_chinese_initial: "tʰ"
middle_chinese_final: "uʌi"
stroke_count: 9
radical: "辵"
skip_number: 3-3-6
grade_level: "2"
pos: "事詞"
english:
  - retreat
  - recede
  - withdraw
羅馬字: tiǝ
諺文: "틔"
joyo_level: "6"
hsk_level: "1"
hanmun_edu_level: "中"
danayo_id: 2248
mc_id: 528
graphemic_classification: "會意"
stand_in: "退"
aliases:
  - 頽
注音: "ㄊㄧㄜ"
date-last-perfect: 2026-07-25
kwin: false
tags:
  - character
boundedness: 80
---
>[!tip] This is a page about the character 退.
>For the word, see [退](words/退.md)

```meta-bind-embed
[[nav/char_info]]
```

## Notes
- 会意 of [[Radical 162|辶]] ("to walk") and [[艮]] (originally 皀, "food vessel" — visually merged with the unrelated character 艮 in the modern glyph, the same shape-drift pattern seen in 即/既/郷) — to leave the table after eating; extended to "to retreat, withdraw." The Shuowen instead reads 彳 ("move") + 夊 ("walk") + 日 ("sun") — "to walk away from the sun."
- [SKIP-3-3-6](lookup/SKIP/SKIP-3/SKIP-3-3-6.md) ([Stroke 09](lookup/Stroke/Stroke%2009.md))
- 528th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 透|tʰ]] + [[Lookup/CC/finals/韻 灰|uʌi]] → [ㄊㄧㄜ](syllables/ㄊㄧㄜ.md)
- [Grade 2](lookup/Grade%202.md), [HSK Beginner](lookup/HSK/HSK%20Beginner.md), [Jōyō - Kyōiku](lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[退]]<rt>ㄊㄧㄜ</rt></ruby> "retreat, recede, withdraw" (stand-in for 退)
- <ruby>[[辞退]]<rt>ㄑㄧㄊㄧㄜ</rt></ruby> "decline, refuse"
- <ruby>[[衰退]]<rt>ㄙ⼔ㄧㄊㄧㄜ</rt></ruby> "decline"
- <ruby>[[退職]]<rt>ㄊㄛㄧㄐㄧㄎ</rt></ruby> "retire, resign"

## Derived Characters
- <ruby>[[腿]]<rt>ㄊㄧㄜ</rt></ruby> "thigh"
