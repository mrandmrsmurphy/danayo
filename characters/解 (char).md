---
mandarin: "jiě"
cantonese: gaai2
korean: "해"
korean_native: "풀"
japanese:
  - KAI
  - GE
japanese_native: さと
vietnamese:
  - giãi
  - giải
middle_chinese_initial: k
middle_chinese_final: "ɣɛ"
stroke_count: 13
radical: "角"
skip_number: 1-7-6
grade_level: "3"
pos: "事詞"
english:
  - explain
羅馬字: gyai
諺文: "걔"
joyo_level: "5"
hsk_level: "1"
hanmun_edu_level: "中"
danayo_id: 3236
mc_id: 466
graphemic_classification: "会意"
stand_in: "解"
aliases:
  - 獬
注音: "ㄍ⼘ㄧ"
date-last-perfect: 2026-07-30
kwin: false
tags:
  - character
boundedness: 100
---
>[!tip] This is a page about the character 解.
>For the word, see [解](words/解.md)
```meta-bind-embed
[[nav/char_info]]
```
## Notes
- 会意 of [[Radical 148|角]] ("horn") + [[刀]] ("knife") + [[牛]] ("cow") — a knife separates a horn from a cow, i.e. "to cut up (an ox)," extended to "loosen, explain, understand." **Corrected here**: the field previously stored `角`, which would incorrectly assert a 形声 relationship with 角 as phonetic — the page's own body already correctly described a 会意 compound.
- [SKIP-1-7-6](../lookup/SKIP/SKIP-1/SKIP-1-7-6.md) ([Stroke 13](../lookup/Stroke/Stroke%2013.md))
- 466th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 見|k]] + [[Lookup/CC/finals/韻 佳開|ɣɛ]] → [ㄍ⼘ㄧ](../syllables/ㄍ⼘ㄧ.md)
- [Grade 3](../lookup/Grade%203.md), [HSK Beginner](../lookup/HSK/HSK%20Beginner.md), [Jōyō - Kyōiku](../lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](../lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[解]]<rt>ㄍ⼘ㄧ</rt></ruby> "explain" (stand-in for 解)
- <ruby>[[了解]]<rt>ㄌ⼘ㄨㄍ⼘ㄧ</rt></ruby> "understand"
- <ruby>[[理解]]<rt>ㄌㄧㄍ⼘ㄧ</rt></ruby> "comprehend; understand"
- <ruby>[[解放]]<rt>ㄍ⼘ㄧㄈㄚㄫ</rt></ruby> "liberate; free"
- <ruby>[[諒解]]<rt>ㄌ⼘ㄫㄍ⼘ㄧ</rt></ruby> "understand; forgive"
- <ruby>[[解決]]<rt>ㄍ⼘ㄧㄎ⼔ㄊ</rt></ruby> "resolve; solve"
- <ruby>[[解禁]]<rt>ㄍ⼘ㄧㄍㄧㄇ</rt></ruby> "lift a ban"
- <ruby>[[解剖]]<rt>ㄍ⼘ㄧㄈㄛㄨ</rt></ruby> "to dissect; anatomy"
- <ruby>[[解釈]]<rt>ㄍ⼘ㄧㄙㄝㄎ</rt></ruby> "interpretation; to interpret"
- <ruby>[[調解]]<rt>ㄐㄨㄛㄍ⼘ㄧ</rt></ruby> "to mediate, settle a dispute"
- <ruby>[[解析]]<rt>ㄍ⼘ㄧㄙㄝㄎ</rt></ruby> "to analyze (technical/mathematical)"

## Derived Characters
- <ruby>[[蟹 (char)|蟹]]<rt>ㄏ⼘ㄧ</rt></ruby> "crab"
