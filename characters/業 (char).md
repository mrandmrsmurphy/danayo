---
mandarin: yè
cantonese: jip6
korean: 업
korean_native: 업
japanese:
  - GYOU
  - GOU
japanese_native:
  - わざ
vietnamese:
  - nghiệp
middle_chinese_initial: ŋ
middle_chinese_final: ɨɐp
stroke_count: 13
radical: 木
skip_number: 2-5-8
grade_level: "1"
pos: 名詞
english:
  - profession
  - business
羅馬字: "'eb"
諺文: 업
joyo_level: "3"
hsk_level: "1"
hanmun_edu_level: 中
danayo_id: 236
mc_id: 601
graphemic_classification: 象形
stand_in: 業
aliases:
  - 业
注音: ㄝㄆ
date-last-perfect: 2026-07-10
kwin: true
tags:
  - character
boundedness: 100
---
>[!tip] This is a page about the character 業.
>For the word, see [業](../words/業.md)
```meta-bind-embed
[[nav/char_info]]
```
## Notes
- [List of 象形](../lookup/List%20of%20象形.md): a beam from which bells and chimes hang, per one reading, or a person lifting such a beam per another — either way, a structure built up for a purpose, extended to "achievement, undertaking."
- [SKIP-2-5-8](../lookup/SKIP/SKIP-2/SKIP-2-5-8.md) ([Stroke 13](../lookup/Stroke/Stroke%2013.md))
- 601st most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 疑|ŋ]] + [[Lookup/CC/finals/韻 業|ɨɐp]] → [ㄝㄆ](../syllables/ㄝㄆ.md)
- [Grade 1](../lookup/Grade%201.md), [HSK Beginner](../lookup/HSK/HSK%20Beginner.md), [Jōyō - Kyōiku](../lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](../lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[工業]]<rt>ㄍㄛㄫㄝㄆ</rt></ruby> "industry"
- <ruby>[[作業]]<rt>ㄐㄚㄎㄝㄆ</rt></ruby> "work; operate; task"
- <ruby>[[業業]]<rt>ㄝㄆㄝㄆ</rt></ruby> "fearfully; apprehensively; lofty" (ideophone)
- <ruby>[[漁業]]<rt>⼄ㄝㄆ</rt></ruby> "fishing industry, fishery"
- <ruby>[[従業]]<rt>ㄐㄛㄫㄝㄆ</rt></ruby> "employment, being engaged in work"

## Chengyu
- <ruby>[[成家立業]]<rt>ㄙㄧㄫㄍㄚㄌㄧㄆ·ㄝㄆ</rt></ruby> "get married and start a career"
- <ruby>[[自業自得]]<rt>ㄐㄧㄜ·ㄝㄆㄐㄧㄜㄊㄜㄎ</rt></ruby> "you reap what you sow; one's own actions return as one's own fate"
