---
mandarin: zhuī
cantonese: zeoi1
korean: 추
korean_native: 쫓을
japanese:
  - TSUI
  - TAI
japanese_native: お
vietnamese:
  - choai
  - truy
middle_chinese_initial: ʈ
middle_chinese_final: ɣiuɪ
stroke_count: 9
radical: 辵
skip_number: 3-3-6
grade_level: "2"
pos: 事詞
english:
  - follow
  - hunt
羅馬字: tui
諺文: 튀
joyo_level: "3"
hsk_level: "2"
hanmun_edu_level: 中
danayo_id: 2247
mc_id: 752
graphemic_classification: 𠂤
stand_in: 追
aliases:
注音: ㄊㄨㄧ
date-last-perfect: 2026-07-25
kwin: false
tags:
  - character
boundedness: 100
---
>[!tip] This is a page about the character 追.
>For the word, see [追](words/追.md)

```meta-bind-embed
[[nav/char_info]]
```
## Notes
- 形声 (OC \*tul, \*truj): semantic [[Radical 162|辵]] ("movement") + phonetic 𠂤 (duī, "mound," no vault page — not to be confused with the derived character [[堆]], itself built from 土+𠂤) — to chase, pursue.
- [SKIP-3-3-6](lookup/SKIP/SKIP-3/SKIP-3-3-6.md) ([Stroke 09](lookup/Stroke/Stroke%2009.md))
- 752nd most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 知|ʈ]] + [[Lookup/CC/finals/韻 脂B合|ɣiuɪ]] → [ㄊㄨㄧ](syllables/ㄊㄨㄧ.md)
- [Grade 2](lookup/Grade%202.md), [Old HSK 2](lookup/HSK/Old%20HSK%202.md), [Jōyō - Kyōiku](lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[追]]<rt>ㄊㄨㄧ</rt></ruby> "follow, hunt" (stand-in for 追)
- <ruby>[[追逐]]<rt>ㄊㄨㄧㄉㄨㄎ</rt></ruby> "chase; pursue"
- <ruby>[[追及]]<rt>ㄊㄨㄧㄍㄨㄆ</rt></ruby> "to pursue and catch; to investigate and hold accountable"
- <ruby>[[追随]]<rt>ㄊㄨㄧㄙㄨㄧ</rt></ruby> "follow, pursue"
- <ruby>[[追遡]]<rt>ㄊㄨㄧㄙㄛ</rt></ruby> "trace back; investigate origins"
- <ruby>[[追求]]<rt>ㄊㄨㄧㄍ⼜ㄛ</rt></ruby> "pursue, search, go after"

## Derived Characters
- <ruby>[[槌 (char)|槌]]<rt>ㄐ⼔ㄧ</rt></ruby> "hammer"
