---
mandarin: bǐ
cantonese: bat1
korean: 필
korean_native: 필
japanese:
  - HITSU
japanese_native: ふで
  - ふで
vietnamese:
  - bút
middle_chinese_initial: p
middle_chinese_final: ɣiɪt
stroke_count: 12
radical: 竹
skip_number: 2-6-6
grade_level: "1"
pos: 名詞
english:
  - pen
  - stylus
羅馬字: pud
諺文: 푿
joyo_level: "3"
hsk_level: "1"
hanmun_edu_level: 中
danayo_id: 218
mc_id: 2106
graphemic_classification: 聿
stand_in: 筆
aliases:
  - 笔
注音: ㄆㄨㄊ
date-last-perfect: 2026-09-06
kwin: false
tags:
  - character
  - hapax
boundedness: 80
---
>[!tip] This is a page about the character 筆.
>For the word, see [筆](words/筆.md)
```meta-bind-embed
[[nav/char_info]]
```
## Notes
- 形声 (OC \*prud): semantic [[Radical 118|竹]] ("bamboo") + phonetic [[聿]] (OC \*b·lud, "writing brush") — 竹 was added later to differentiate a bamboo-made brush from the original 聿, which already meant "writing brush" on its own.
- [SKIP-2-6-6](lookup/SKIP/SKIP-2/SKIP-2-6-6.md) ([Stroke 12](lookup/Stroke/Stroke%2012.md))
- 2106th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 幫|p]] + [[Lookup/CC/finals/韻 質開|ɣiɪt]] → [ㄆㄨㄊ](syllables/ㄆㄨㄊ.md)
- [Grade 1](lookup/Grade%201.md), [HSK Beginner](lookup/HSK/HSK%20Beginner.md), [Jōyō - Kyōiku](lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](lookup/Korean/Korean%20MS.md)
- Altered to take advantage of a vacant syllable.

## Words
- <ruby>[[筆]]<rt>ㄆㄨㄊ</rt></ruby> "pen; stylus" (stand-in for 筆)
- <ruby>[[擱筆]]<rt>ㄍㄚㄎㄆㄨㄊ</rt></ruby> "to lay down the pen; to stop writing"
- <ruby>[[筆記]]<rt>ㄆㄨㄊㄍㄧ</rt></ruby> "to take notes"
- <ruby>[[筆画]]<rt>ㄆㄨㄊㄏ⺢ㄎ</rt></ruby> "stroke"
- <ruby>[[鉛筆]]<rt>⼶ㄋㄆㄨㄊ</rt></ruby> "pencil"
