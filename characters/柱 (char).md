---
mandarin: "zhù"
cantonese: cyu5
korean: "주"
korean_native: "기둥"
japanese:
  - CHUU
japanese_native: はしら
vietnamese:
  - trụ
middle_chinese_initial: "ɖ"
middle_chinese_final: "ɨo"
stroke_count: 9
radical: "木"
skip_number: 1-4-5
grade_level: "4"
pos: 名詞
english:
  - pillar
羅馬字: ju
諺文: "주"
joyo_level: "3"
hsk_level: "3"
hanmun_edu_level: "高等"
danayo_id: 4136
mc_id: 1398
graphemic_classification: "主"
stand_in: "柱"
aliases:
注音: "ㄐㄨ"
date-last-perfect: 2026-07-10
kwin: true
tags:
  - character
boundedness: 75
---
>[!tip] This is a page about the character 柱.
>For the word, see [柱](../words/柱.md)

```meta-bind-embed
[[nav/char_info]]
```

## Notes
- 形声 (OC \*doʔ): semantic [[Radical 075|木]] ("wood") + phonetic [[主]] ("master, lord") — a wooden post that supports, like 拄 ("to prop up"), a cognate sharing the same root.
- [SKIP-1-4-5](../lookup/SKIP/SKIP-1/SKIP-1-4-5.md) ([Stroke 09](../lookup/Stroke/Stroke%2009.md))
- 1398th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 澄|ɖ]] + [[Lookup/CC/finals/韻 虞|ɨo]] → [ㄐㄨ](../syllables/ㄐㄨ.md)
- [Grade 4](../lookup/Grade%204.md), [Old HSK 3](../lookup/HSK/Old%20HSK%203.md), [Jōyō - Kyōiku](../lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean HS](../lookup/Korean/Korean%20HS.md)

## Words
- <ruby>[[緑柱石]]<rt>ㄌㄛㄎㄐㄨㄙㄝㄎ</rt></ruby> "beryl"
- <ruby>[[緑柱素]]<rt>ㄌㄛㄎㄐㄨㄙㄛ</rt></ruby> "beryllium"
