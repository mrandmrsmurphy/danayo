---
mandarin: sù
cantonese: cuk1
korean: 속
korean_native: 빠를
japanese:
  - SOKU
japanese_native: すみ
vietnamese:
  - tốc
  - rốc
middle_chinese_initial: s
middle_chinese_final: uk
stroke_count: 11
radical: 辵
skip_number: 3-4-7
grade_level: "2"
pos: 性詞
english:
  - quick
  - fast
羅馬字: sog
諺文: 속
joyo_level: "3"
hsk_level: "2"
hanmun_edu_level: 中
danayo_id: 2249
mc_id: 1363
graphemic_classification: 束
stand_in: 速
aliases:
注音: ㄙㄛㄎ
date-last-perfect: 2026-07-25
kwin: true
tags:
  - character
boundedness: 80
---
>[!tip] This is a page about the character 速.
>For the word, see [速](words/速.md)

```meta-bind-embed
[[nav/char_info]]
```

## Notes
- 形声 (OC \*sloːɡ): semantic [[Radical 162|辵]] ("motion") + phonetic [[束 (char)|束]] (OC \*hljoɡ) — fast, rapid motion.
- [SKIP-3-4-7](lookup/SKIP/SKIP-3/SKIP-3-4-7.md) ([Stroke 11](lookup/Stroke/Stroke%2011.md))
- 1363rd most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 心|s]] + [[Lookup/CC/finals/韻 屋一|uk]] → [ㄙㄛㄎ](syllables/ㄙㄛㄎ.md)
- [Grade 2](lookup/Grade%202.md), [Old HSK 2](lookup/HSK/Old%20HSK%202.md), [Jōyō - Kyōiku](lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[速]]<rt>ㄙㄛㄎ</rt></ruby> "quick, fast" (stand-in for 速)
- <ruby>[[迅速]]<rt>ㄙ⼜ㄋㄙㄛㄎ</rt></ruby> "rapid; swift; prompt"
- <ruby>[[急速]]<rt>ㄍㄧㄆㄙㄛㄎ</rt></ruby> "rapid; swift"
- <ruby>[[速様]]<rt>ㄙㄛㄎ⼘ㄫ</rt></ruby> "quickly"
