---
mandarin: yàng
cantonese: joeng4
korean: 양
korean_native: 모양
japanese:
  - YOU
  - SHOU
japanese_native: さま
vietnamese:
  - dạng
  - dáng
  - nhàng
middle_chinese_initial: ŋ
middle_chinese_final: ɨɐŋ
stroke_count: 14
radical: 木
skip_number: 1-4-10
grade_level: "3"
pos: 格助詞
english:
  - -ly (adverbializer)
羅馬字: "'yang"
諺文: 양
joyo_level: "3"
hsk_level: "1"
hanmun_edu_level: 高等
danayo_id: 3166
mc_id: 9173
graphemic_classification: 羊
stand_in: 様
aliases:
  - 樣
  - 样
注音: ⼘ㄫ
date-last-perfect: 2026-07-29
kwin: true
tags:
  - character
boundedness: 
---
>[!tip] This is a page about the character 様.
>For the word, see [様](words/様.md)

```meta-bind-embed
[[nav/char_info]]
```

## Notes
- 形声 (OC *laŋs): semantic [[Radical 075|木]] ("tree, wood") + phonetic 羕 (no character page exists for it in this vault; part of the same broad *laŋ phonetic series as [[羊 (char)|羊]] itself, whose own Dan'a'yo syllable ⼘ㄫ matches 様's exactly, so the stored field cites 羊 directly) — a kind of tree, later borrowed for "appearance, manner, way," and (in this vault) the -ly adverbializer / manner-case marker.
- [SKIP-1-4-10](../lookup/SKIP/SKIP-1/SKIP-1-4-10.md) ([Stroke 14](../lookup/Stroke/Stroke%2014.md))
- 9173rd most used character in Classical Chinese (outside the locally-checkable `CC 0000–3000` range, trusted verbatim per the checklist's own policy). Ancient [[Lookup/CC/initials/聲 疑|ŋ]] + [[Lookup/CC/finals/韻 陽開|ɨɐŋ]] → [⼘ㄫ](../syllables/⼘ㄫ.md)
- [Grade 3](../lookup/Grade%203.md), [HSK Beginner](../lookup/HSK/HSK%20Beginner.md), [Jōyō - Kyōiku](../lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean HS](../lookup/Korean/Korean%20HS.md)

## Words

### Correlatives
- <ruby>[[此様]]<rt>ㄑㄜ⼘ㄫ</rt></ruby> "this way, like this"
- <ruby>[[其様]]<rt>ㄍㄜ⼘ㄫ</rt></ruby> "that way, in that manner"
- <ruby>[[彼様]]<rt>ㄅㄜ⼘ㄫ</rt></ruby> "in that other way, in that fashion"
- <ruby>[[何様]]<rt>ㄏㄚ⼘ㄫ</rt></ruby> "how, in what way"
- <ruby>[[毎様]]<rt>ㄇㄛㄧ⼘ㄫ</rt></ruby> "each way, in each manner"
- <ruby>[[某様]]<rt>ㄇㄛㄨ⼘ㄫ</rt></ruby> "somehow, in some way"
- <ruby>[[皆様]]<rt>ㄍ⼶⼘ㄫ</rt></ruby> "in every way, in all respects"

### Manner adverbs
- <ruby>[[忽然様]]<rt>ㄏㄛㄊㄋ⼶ㄋ⼘ㄫ</rt></ruby> "suddenly"
- <ruby>[[徐様]]<rt>ㄙ⼄·⼘ㄫ</rt></ruby> "slowly"
- <ruby>[[速様]]<rt>ㄙㄛㄎ⼘ㄫ</rt></ruby> "quickly"
- <ruby>[[明様]]<rt>ㄇ⼶ㄫ⼘ㄫ</rt></ruby> "brightly, in a bright way"
- <ruby>[[太様]]<rt>ㄊㄚㄧ·⼘ㄫ</rt></ruby> "greatly, exceedingly"
- <ruby>[[甚様]]<rt>ㄙㄧㄇ⼘ㄫ</rt></ruby> "what manner, of what sort, how"
- <ruby>[[予様]]<rt>⼄·⼘ㄫ</rt></ruby> "beforehand"
- <ruby>[[有様]]<rt>⼜⼘ㄫ</rt></ruby> "forebearingly, calmly (dated)"

### Other
- <ruby>[[様子]]<rt>⼘ㄫㄐㄜ</rt></ruby> "appearances, manner, circumstances, state of things"
- <ruby>[[様式]]<rt>⼘ㄫㄙㄧㄎ</rt></ruby> "type, style, pattern"
- <ruby>[[様態格]]<rt>⼘ㄫㄊㄚㄧㄍㄚㄎ</rt></ruby> "manner case"
- <ruby>[[多様]]<rt>ㄉㄜ⼘ㄫ</rt></ruby> "diverse, diversity"
