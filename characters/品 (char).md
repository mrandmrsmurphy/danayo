---
mandarin: pǐn
cantonese: ban2
korean: 품
korean_native: 물건
japanese:
  - HIN
  - HON
japanese_native: しな
vietnamese:
  - phẩm
middle_chinese_initial: pʰ
middle_chinese_final: ɣiɪm
stroke_count: 9
radical: 口
skip_number: 2-3-6
grade_level: "3"
pos: 名詞
english:
  - article
  - item
羅馬字: pum
諺文: 품
joyo_level: "3"
hsk_level: "2"
hanmun_edu_level: 中
danayo_id: 3059
mc_id: 1854
graphemic_classification: 會意
stand_in: 品
aliases:
注音: ㄆㄨㄇ
date-last-perfect: 2026-07-26
kwin: true
tags:
  - character
boundedness: 75
---
>[!tip] This is a page about the character 品.
>For the word, see [品](words/品.md)
```meta-bind-embed
[[nav/char_info]]
```
## Notes
- 会意: Triplication of [[Radical 030|口]] ("mouth"), representing various objects — a multiplicity of items/things, underlying the later senses "article, product" and "grade, rank" (things sorted into categories). Also occurs in 區 (no vault page).
- [SKIP-2-3-6](lookup/SKIP/SKIP-2/SKIP-2-3-6.md) ([Stroke 09](lookup/Stroke/Stroke%2009.md))
- 1,854th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 滂|pʰ]] + [[Lookup/CC/finals/韻 侵B|ɣiɪm]] → [ㄆㄨㄇ](syllables/ㄆㄨㄇ.md)
- [Grade 3](lookup/Grade%203.md), [Old HSK 2](lookup/HSK/Old%20HSK%202.md), [Jōyō - Kyōiku](lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[品]]<rt>ㄆㄨㄇ</rt></ruby> "article; item" (stand-in for 品)
- <ruby>[[新品]]<rt>ㄙㄧㄋㄆㄨㄇ</rt></ruby> "new products; new arrivals"
- <ruby>[[製品]]<rt>ㄐㄝㄆㄨㄇ</rt></ruby> "product; produce; goods"
- <ruby>[[貢品]]<rt>ㄍㄛㄫㄆㄨㄇ</rt></ruby> "tribute; tribute goods"
- <ruby>[[品詞]]<rt>ㄆㄨㄇㄙㄚ</rt></ruby> "part of speech"
- <ruby>[[品目]]<rt>ㄆㄨㄇㄇㄨㄎ</rt></ruby> "article; item; inventory"

## Derived Characters
- <ruby>[[臨]]<rt>ㄌㄧㄇ</rt></ruby> "draw near; approach"
