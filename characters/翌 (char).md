---
mandarin: yì
cantonese: jik6
korean: 익
korean_native: 다음날
japanese:
  - YOKU
japanese_native: ø
vietnamese:
  - dực
middle_chinese_initial: j
middle_chinese_final: ɨk
stroke_count: 11
radical: 羽
skip_number: 2-6-5
grade_level: 先進
pos: 修飾語
english:
  - the following
  - after
  - next
羅馬字: "'ig"
諺文: 익
joyo_level: "6"
hsk_level: "無"
hanmun_edu_level: 名
danayo_id: 7541
mc_id: 5320
graphemic_classification: 立
stand_in: 翌
aliases:
注音: ㄧㄎ
kwin: true
tags:
  - character
boundedness: 75
date-last-perfect: 2026-09-05
---
>[!tip] This is a page about the character 翌.
>For the word, see [翌](../words/翌.md)

```meta-bind-embed
[[nav/char_info]]
```

## Notes
- 形声 (OC *lɯɡ): semantic [[Radical 124|羽]] ("feather") + phonetic [[立 (char)|立]] — 立 here is a graphic truncation of 昱 ("bright, the next day," OC *luɡ, the true phonetic match), which lost its 日 component in this glyph, leaving only 立 visible; "the following, next."
- [SKIP-2-6-5](Lookup/SKIP/SKIP-2/SKIP-2-6-5.md) ([Stroke 11](Lookup/Stroke/Stroke%2011.md))
- 5320th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 以|j]] + [[Lookup/CC/finals/韻 職開|ɨk]] → [ㄧㄎ](syllables/ㄧㄎ.md)
- [Grade Advanced](Lookup/Grade%20Advanced.md), [HSK No](Lookup/HSK/HSK%20No.md), [Jōyō - Kyōiku](Lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean Name ㅇ](Lookup/Korean/Korean%20Name%20ㅇ.md)

## Words
- <ruby>[[翌]]<rt>ㄧㄎ</rt></ruby> "the following" (stand-in for 翌)
- <ruby>[[翌月]]<rt>ㄧㄎ⼔ㄊ</rt></ruby> "the following month"
