---
mandarin: "shé"
cantonese: sit3
korean: "설"
korean_native: "혀"
japanese:
  - ZETSU
  - SETSU
japanese_native: した
vietnamese:
  - thiệt
  - thịt
middle_chinese_initial: "d͡ʑ"
middle_chinese_final: "iᴇt"
stroke_count: 6
radical: "舌"
skip_number: 2-3-3
grade_level: "3"
pos: 名詞
english:
  - tongue
  - clapper
羅馬字: sed
諺文: "섣"
joyo_level: "6"
hsk_level: "2"
hanmun_edu_level: "中"
danayo_id: 3229
mc_id: 1457
graphemic_classification: "象形"
stand_in: "舌"
aliases:
注音: "ㄙㄝㄊ"
date-last-perfect: 2026-07-30
kwin: true
tags:
  - character
boundedness: 35
---
>[!tip] This is a page about the character 舌.
>For the word, see [舌](words/舌.md)
```meta-bind-embed
[[nav/char_info]]
```
## Notes
- [List of 象形](../lookup/List%20of%20象形.md): a snake's tongue emerging upwards from a mouth ([[口]]) — oracle-bone script showed a mouth plus a forked snake tongue, later variants adding a horizontal stroke at the base that often evolved into a second fork (double-forked).
- [SKIP-2-3-3](../lookup/SKIP/SKIP-2/SKIP-2-3-3.md) ([Stroke 06](../lookup/Stroke/Stroke%2006.md))
- 1457th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 禪|d͡ʑ]] + [[Lookup/CC/finals/韻 薛A三開|iᴇt]] → [ㄙㄝㄊ](../syllables/ㄙㄝㄊ.md)
- [Grade 3](../lookup/Grade%203.md), [Old HSK 2](../lookup/HSK/Old%20HSK%202.md), [Jōyō - Kōtō](../lookup/Japanese/Jōyō%20-%20Kōtō.md), [Korean MS](../lookup/Korean/Korean%20MS.md) — promoted from Korean HS to MS in 2000.

## Words
- <ruby>[[舌根]]<rt>ㄙㄝㄊㄍㄨㄋ</rt></ruby> "root of the tongue, taste faculty"
- <ruby>[[尭舌]]<rt>⼘ㄨㄙㄝㄊ</rt></ruby> "talkative; MC-ing"

## Derived Characters
Each of these descends from 𠯑 (no character page in this vault), which visually evolved into the modern glyph 舌 shared by all — the same "recognizable descendant of an unlinkable ancestor" pattern as 悟/五 or 栽/才.
- <ruby>[[活 (char)|活]]<rt>ㄏ⺢ㄊ</rt></ruby> "alive"
- <ruby>[[刮]]<rt>ㄍ⺢ㄊ</rt></ruby> "scrape"
- <ruby>[[括]]<rt>ㄍ⺢ㄊ</rt></ruby> "embrace; include"
- <ruby>[[筈 (char)|筈]]<rt>ㄍ⺢ㄊ</rt></ruby> "nock"
- <ruby>[[蛞]]<rt>ㄎ⺢ㄊ</rt></ruby> "slug"
- <ruby>[[聒]]<rt>ㄍ⺢ㄆ</rt></ruby> "noisy"
