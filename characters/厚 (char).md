---
mandarin: hòu
cantonese: hau5
korean: 후
korean_native: 두터울
japanese:
  - KOU
japanese_native: あつ
vietnamese:
  - hậu
middle_chinese_initial: ɣ
middle_chinese_final: əu
stroke_count: 9
radical: 厂
skip_number: 3-2-7
grade_level: "3"
pos: 性詞
english:
  - thick
羅馬字: hou
諺文: 홋
joyo_level: "5"
hsk_level: "2"
hanmun_edu_level: 中
danayo_id: 3044
mc_id: 456
graphemic_classification: 会意
stand_in: 厚
aliases:
注音: ㄏㄛㄨ
date-last-perfect: 2026-07-26
kwin: false
tags:
  - character
boundedness: 90
---
>[!tip] This is a page about the character 厚.
>For the word, see [厚](words/厚.md)

```meta-bind-embed
[[nav/char_info]]
```

## Notes
- 会意 of [[Radical 027|厂]] ("cliff; cave on a cliff") and 𣆪 ("jug" — per Shuowen, an inverted 亯; no vault page) — the semantic logic bridging the components to "thick, substantial" is not spelled out in the source; treated as an attested component breakdown rather than a fully transparent derivation.
- [SKIP-3-2-7](lookup/SKIP/SKIP-3/SKIP-3-2-7.md) ([Stroke 09](lookup/Stroke/Stroke%2009.md))
- 456th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 匣|ɣ]] + [[Lookup/CC/finals/韻 侯|əu]] → [ㄏㄛㄨ](syllables/ㄏㄛㄨ.md)
- [Grade 3](lookup/Grade%203.md), [Old HSK 2](lookup/HSK/Old%20HSK%202.md), [Jōyō - Kyōiku](lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](lookup/Korean/Korean%20MS.md)

## Words
- <ruby>[[厚]]<rt>ㄏㄛㄨ</rt></ruby> "thick"
- <ruby>[[濃厚]]<rt>ㄋㄛㄫㄏㄛㄨ</rt></ruby> "concentrated; thick; dense; strong"
- <ruby>[[敦厚]]<rt>ㄉㄛㄋㄏㄛㄨ</rt></ruby> "honest; candid; sincere"
- <ruby>[[厚顔]]<rt>ㄏㄛㄨㄚㄋ</rt></ruby> "brazen-faced; impudent; shameless"

## Chengyu
- <ruby>[[厚顔無恥]]<rt>ㄏㄛㄨ·ㄚㄋㄇㄜㄑㄧ</rt></ruby> "shameless and brazen"
