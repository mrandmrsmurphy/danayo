---
mandarin: "dān"
cantonese: daam1
korean: "탐"
korean_native: "처질"
japanese:
  - TAN
japanese_native: ふけ
vietnamese:
  - đam
middle_chinese_initial: t
middle_chinese_final: "ʌm"
stroke_count: 10
radical: "耳"
skip_number: 1-6-4
grade_level: "先進"
pos: 事詞
english:
  - indulge in
羅馬字: dom
諺文: "돔"
joyo_level: "日本人名用漢字"
hsk_level: "無"
hanmun_edu_level: "名"
danayo_id: 7437
mc_id: 3012
graphemic_classification: "冘"
stand_in: "耽"
aliases:
  - 躭
注音: "ㄉㄛㄇ"
kwin: false
tags:
  - character
boundedness: 75
date-last-perfect: 2026-08-15
---
>[!tip] This is a page about the character 耽.
>For the word, see [耽](words/耽.md)
```meta-bind-embed
[[nav/char_info]]
```
## Notes
- 形声 (OC \*ʔl'uːm): semantic [[Radical 128|耳]] ("ear") + phonetic [[冘]] (OC \*lu, \*lum) — to indulge in, be absorbed in.
- [SKIP-1-6-4](Lookup/SKIP/SKIP-1/SKIP-1-6-4.md) ([Stroke 10](Lookup/Stroke/Stroke%2010.md))
- 3012th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 端|t]] + [[Lookup/CC/finals/韻 覃|ʌm]] → [ㄉㄛㄇ](syllables/ㄉㄛㄇ.md)
- [Grade Advanced](Lookup/Grade%20Advanced.md), [HSK No](Lookup/HSK/HSK%20No.md), [Jinmeiyō](Lookup/Japanese/Jinmeiyō.md), [Korean Name ㅌ](Lookup/Korean/Korean%20Name%20ㅌ.md)

## Words
- <ruby>[[耽]]<rt>ㄉㄛㄇ</rt></ruby> "indulge in"
- <ruby>[[耽耽]]<rt>ㄉㄛㄇㄉㄛㄇ</rt></ruby> "glaring covetously" (alias: 眈眈)

## Chengyu
- <ruby>[[虎視耽耽]]<rt>ㄏㄛㄙㄧㄜㄉㄛㄇㄉㄛㄇ</rt></ruby> "to eye covetously like a prowling tiger"
