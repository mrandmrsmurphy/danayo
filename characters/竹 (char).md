---
mandarin: "zhú"
cantonese: zuk1
korean: "죽"
korean_native: "대"
japanese:
  - CHIKU
japanese_native: たけ
vietnamese:
  - trúc
middle_chinese_initial: "ʈ"
middle_chinese_final: "ɨuk"
stroke_count: 6
radical: "竹"
skip_number: 1-3-3
grade_level: "1"
pos: "名詞"
english:
  - bamboo
羅馬字: jug
諺文: "죽"
joyo_level: "1"
hsk_level: "2"
hanmun_edu_level: "中"
danayo_id: 79
mc_id: 1130
graphemic_classification: "象形"
stand_in: "竹"
注音: "ㄐㄨㄎ"
date-last-perfect: 2026-07-16
kwin: true
tags:
  - character
boundedness: 30
---
>[!tip] This is a page about the character 竹.
>For the word, see [竹](words/竹.md)
>For the radical, see [Radical 118](../lookup/Radicals/Radical%20118.md)
```meta-bind-embed
[[nav/char_info]]
```
## Notes
- [List of 象形](../lookup/List%20of%20象形.md): two bamboo stalks with leaves; earlier forms resembled 个+个, the current form resembles ケ+ケ.
- [SKIP-1-3-3](../lookup/SKIP/SKIP-1/SKIP-1-3-3.md) ([Stroke 06](../lookup/Stroke/Stroke%2006.md))
- 1130th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 知|ʈ]] + [[Lookup/CC/finals/韻 屋三|ɨuk]] → [ㄐㄨㄎ](../syllables/ㄐㄨㄎ.md)
- [Grade 1](../lookup/Grade%201.md), [Old HSK 2](../lookup/HSK/Old%20HSK%202.md), [Jōyō - Kyōiku](../lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](../lookup/Korean/Korean%20MS.md)
## Words
- <ruby>[[竹筍]]<rt>ㄐㄨㄎㄙ⼜ㄊ</rt></ruby> "bamboo shoot"
- <ruby>[[篠竹]]<rt>ㄙㄛㄨㄐㄨㄎ</rt></ruby> "dwarf bamboo"
- <ruby>[[松竹梅]]<rt>ㄙ⼄ㄫㄐㄨㄎㄇㄛㄧ</rt></ruby> "Three Friends of Winter"
- <ruby>[[竿竹]]<rt>ㄍㄚㄋㄐㄨㄎ</rt></ruby> "bamboo pole"
