...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ensure there is a space after ':' for a given YAML property in Markdown front matter."
    )
    parser.add_argument(
        "--commit",
        action="store_true",
...
def fix_property_spacing_in_front_matter(front_matter: str, prop: str) -> tuple[str, list[str]]:
    lines = front_matter.splitlines(keepends=True)
    changes: list[str] = []

    pattern = re.compile(rf"^({re.escape(prop)}):(\")([^\n\r]*)(\r?\n?)$")
...
def process_file(path: Path, prop: str, commit: bool) -> bool:
    ...
    new_front_matter, changes = fix_property_spacing_in_front_matter(front_matter, prop)
