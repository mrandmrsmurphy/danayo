---
mandarin: "qián"
cantonese: cin4
korean: "전"
korean_native: "앞"
japanese:
  - ZEN
  - SEN
japanese_native: まえ
vietnamese:
  - tiền
  - tèn
middle_chinese_initial: "d͡z"
middle_chinese_final: en
stroke_count: 9
radical: "刀"
skip_number: 2-3-6
grade_level: "1"
pos: 名詞
english:
  - front
  - before
  - preceding
羅馬字: jen
諺文: "전"
joyo_level: "2"
hsk_level: "1"
hanmun_edu_level: "中"
danayo_id: 179
mc_id: 235
graphemic_classification: "歬"
stand_in: "前"
aliases:
  - 揃
注音: "ㄐㄝㄋ"
date-last-perfect: 2026-07-23
kwin: true
tags:
  - character
boundedness: 100
---
>[!tip] This is a page about the character 前.
>For the word, see [前](words/前.md)

```meta-bind-embed
[[nav/char_info]]
```

## Notes
- 会意: original form [[歬]] combined 止 ("foot") + [[舟]] ("boat") — a foot on a boat, moving forward. In the modern glyph 止 simplified to 䒑 and 舟 to 月, with [[Radical 018|刀]] ("knife") added later — that addition now represents [[剪 (char)|剪]] ("to cut"), the character 前 itself absorbed the "front, before" sense while shedding the cutting meaning.
- [SKIP-2-3-6](lookup/SKIP/SKIP-2/SKIP-2-3-6.md) ([Stroke 09](lookup/Stroke/Stroke%2009.md))
- 235th most used character in Classical Chinese. Ancient [[Lookup/CC/initials/聲 從|d͡z]] + [[Lookup/CC/finals/韻 先開|en]] → [ㄐㄝㄋ](syllables/ㄐㄝㄋ.md)
- [Grade 1](lookup/Grade%201.md), [HSK Beginner](lookup/HSK/HSK%20Beginner.md), [Jōyō - Kyōiku](lookup/Japanese/Jōyō%20-%20Kyōiku.md), [Korean MS](lookup/Korean/Korean%20MS.md)
## Words
- <ruby>[[前]]<rt>ㄐㄝㄋ</rt></ruby> "front; before" (stand-in for 前)
- <ruby>[[以前]]<rt>ㄧㄐㄝㄋ</rt></ruby> "before; ago"
- <ruby>[[目前]]<rt>ㄇㄨㄎㄐㄝㄋ</rt></ruby> "present; right before one's eyes"
- <ruby>[[前置詞]]<rt>ㄐㄝㄋㄑㄧㄙㄚ</rt></ruby> "preposition"
- <ruby>[[前後]]<rt>ㄐㄝㄋㄏㄛㄨ</rt></ruby> "front and rear; before and after; from beginning to end"
- <ruby>[[従前]]<rt>ㄐㄛㄫㄐㄝㄋ</rt></ruby> "formerly, previously"
- <ruby>[[前兆]]<rt>ㄐㄝㄋㄐㄚㄨ</rt></ruby> "premonitory sign; harbinger; omen"
- <ruby>[[前年]]<rt>ㄐㄝㄋㄋㄝㄋ</rt></ruby> "previous year"
- <ruby>[[前提]]<rt>ㄐㄝㄋㄙㄝ</rt></ruby> "prerequisite; premise"
- <ruby>[[前日]]<rt>ㄐㄝㄋㄋㄧㄊ</rt></ruby> "prior day"
- <ruby>[[前月]]<rt>ㄐㄝㄋ⼔ㄊ</rt></ruby> "previous month"
- <ruby>[[前週]]<rt>ㄐㄝㄋㄐㄨㄛ</rt></ruby> "previous week"
- <ruby>[[午前]]<rt>ㄛㄐㄝㄋ</rt></ruby> "a.m."
- <ruby>[[眼前]]<rt>ㄚㄋㄐㄝㄋ</rt></ruby> "before one's eyes"
- <ruby>[[空前]]<rt>ㄎㄛㄫㄐㄝㄋ</rt></ruby> "unprecedented; as never before"
- <ruby>[[前世紀]]<rt>ㄐㄝㄋㄙㄝㄍㄧ</rt></ruby> "previous century"
- <ruby>[[紀元前]]<rt>ㄍㄧ⼔ㄋㄐㄝㄋ</rt></ruby> "B.C.; B.C.E."
## Chengyu
- <ruby>[[空前絶後]]<rt>ㄎㄛㄫㄐㄝㄋㄐ⼔ㄊㄏㄨㄛ</rt></ruby> "unprecedented and unrepeatable; without parallel before or after"
- <ruby>[[珠投猪前]]<rt>ㄐㄨㄉㄛㄨㄐㄛㄐㄝㄋ</rt></ruby> "throwing pearls before swine"
## Derived Characters
- <ruby>[[剪 (char)|剪]]<rt>ㄐㄝㄋ</rt></ruby> "scissors"
- <ruby>[[煎]]<rt>ㄐㄝㄋ</rt></ruby> "to boil; to fry"
- <ruby>[[箭]]<rt>ㄐㄝㄋ</rt></ruby> "arrow"
