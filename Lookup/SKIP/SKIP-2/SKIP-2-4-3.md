---
stroke_count: 7
date-last-perfect: 2026-06-13
size: 15
skip_number: 2-4-3
tags: [lookup]

---
> SKIP : 2 : [4](lookup/SKIP/SKIP-2/SKIP-2-4.md)
> [Stroke 07](lookup/Stroke/Stroke%2007.md)

## Characters
1. <ruby>[否](characters/否%20(char).md)<rt>ㄈㄚㄨ</rt></ruby> "no"
2. <ruby>[吾](/characters/吾.md)<rt>ㄚ</rt></ruby> "I"
3. <ruby>[告](/characters/告.md)<rt>ㄍㄚㄨ</rt></ruby> "inform"
4. <ruby>[呑](/characters/呑.md)<rt>ㄊㄚㄋ</rt></ruby> "swallow; absorb; annex; engulf"
5. <ruby>[妥](/characters/妥.md)<rt>ㄊㄚ</rt></ruby> "appropriate; proper"
6. <ruby>[孚](/characters/孚.md)<rt>ㄈㄨ</rt></ruby> "confident; sincere"
7. <ruby>[巠](/characters/巠.md)<rt>ㄍㄝㄫ</rt></ruby> "underground stream"
8. <ruby>[弄](characters/弄%20(char).md)<rt>ㄌㄛㄫ</rt></ruby> "tamper with; mess with"
9. <ruby>[忠](/characters/忠.md)<rt>ㄊㄨㄫ</rt></ruby> "loyalty; devotion; fidelity"
10. <ruby>[旱](/characters/旱.md)<rt>ㄏㄚㄋ</rt></ruby> "drought"
11. <ruby>[李](/characters/李.md)<rt>ㄌㄧ</rt></ruby> "plum"
12. <ruby>[杏](/characters/杏.md)<rt>ㄏㄚㄫ</rt></ruby> "apricot"
13. <ruby>[肖](/characters/肖.md)<rt>ㄙ⼄ㄨ</rt></ruby> "portrait"
14. <ruby>[麦](/characters/麦.md)<rt>ㄇㄚㄎ</rt></ruby> "wheat; barley; oats"
15. <ruby>[㐬](/characters/㐬.md)<rt>ㄌ⼜ㄛ</rt></ruby> "strings of jade hanging from a crown; ornamental streamers beneath a banner or flag"

## Datacheck
```dataview
TABLE 注音 AS "Sound", english AS "en"
FROM "characters"
WHERE skip_number = "2-4-3"
SORT file.name ASC
```
