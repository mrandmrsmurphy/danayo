...
- Caution: word-final voiced obstruents (d/g/b) on a character's own stored `羅馬字`/`注音` legitimately devoice to (t/k/p) when that character leads a compound immediately followed by another consonant-initial syllable — e.g. 日 (nid) → 日素 (nit-so), 月 (wed) → 月素 (wet-so), 緑 (log) → 緑柱素 (lok-ju-so). This is a real Dan'a'yo sandhi rule, not drift — don't "fix" it back to the bare constituent reading.
