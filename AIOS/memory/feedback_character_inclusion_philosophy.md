...
**The alias definition itself is Wiktionary's, and it is wildly permissive (user, 2026-09-13).** When deciding whether a glyph can fold into an existing character's `aliases:`, the standard is Wiktionary's loose "variant" notion — traditional/simplified pairs, 異體字 from zdic/Kangxi ("X 通作 Y"), and even phonology-only stand-ins where the vault has already designated a character as the family's canonical phonology bearer. Two precedents set the same day: 壕→豪 (a real variant chain: Kangxi 壕 通作濠, and 濠 was already folded into 豪 — only the "moat/trench" sense branch, not 豪's core meaning) and 櫓→芦 (**zero shared senses** — 芦 "reed" vs 櫓 "scull/watchtower/shield" — but the vault had already documented 芦 as 櫓's phonology stand-in in the word page and Hyōgai.md; folding it simply made the character page consistent). The earlier "matching sound *and* transparent meaning link" test above is the *safe* default, not a hard floor — the user will explicitly approve sense-mismatched folds when the phonology-carrier relationship is already established. What still blocks a fold: the partner being a mere phonetic *parent* (丁 for 酊, 旬 for 詢) or a compound partner (螟 for 蛉), or a prior explicit ruling against the fold (媚 was ruled *not* an alias of 眉, recorded on 眉's own page).
