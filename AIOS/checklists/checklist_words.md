...
**Format**: a standalone bullet at the end of `## Notes`, after the substantive paragraphs: `- See [DomainName](../lexipedia/DomainName.md).` — established 2026-09-18 while backfilling all 70 words linked from [[lexipedia/Numbers]]. Use the real relative path with `../lexipedia/`, not a bare `lexipedia/DomainName.md` — several older organic backlinks in the corpus are missing the `../` and silently resolve to a nonexistent `words/lexipedia/` path; don't copy that mistake.
