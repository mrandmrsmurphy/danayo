...
Asked directly whether every one of the 118 abbreviation characters has a unique reading. It did not, at the time — found via the same script approach, checking each abbreviation glyph's own stored `注音`:
- **鬼 (cobalt, 鬼金) / 奇 (xenon, 奇素)** — both ㄍㄨㄧ. Both were genuine, independently well-justified semantic calques (long documented etymological reasoning on each word page, no false claims, no reference to each other) — a real naming collision, not a silent bug.
- **露 (ruthenium, 露金) / 鈩 (rutherfordium)** — resolved as a side effect of the 拉/鈩 fix above.

**User then asked to rename xenon to resolve the 鬼/奇 collision** (2026-09-17): renamed `words/奇素.md` → `words/異素.md`. 異 ("different, foreign") is if anything a *tighter* semantic match to Xenon's Greek root ξένος ("foreigner, stranger") than 奇 ("strange, odd, rare") was, and its own reading (ㄧ) collides with nothing else in the table — confirmed by re-running the full 118-character uniqueness check after the swap: **zero collisions remain.** 奇素 kept as an alias. Updated: `characters/奇.md` (removed the xenon citation), `characters/素.md` and `characters/異.md` (Words citations swapped/added), `characters/異.md` given the dedicated "abbreviation for xenon" Notes bullet per [[feedback_element_abbreviation_characters]], and both copies of the periodic table (see below).

## A silently-drifting duplicate table found along the way

While doing the rename, discovered `lookup/List of 新語.md` ("List of Neologisms") carries its own **full independent copy** of the Main/D-Block/F-Block tables, unlinked to `lexipedia/Periodic Table.md` and never updated by any of this project's earlier fixes. Diffed the two programmatically and found it was still showing **5 of the exact same typo values already fixed on the primary page**: 奇素, 造金, 薔薇金, 始金, 加多金 — i.e. this second copy had been frozen at a pre-fix snapshot the whole time and nobody had thought to check it. Synced all 5, plus the xenon rename, and re-verified zero diffs remain between the two tables. **Lesson: before declaring a periodic-table (or any duplicated-content) fix complete, grep the whole vault for other copies of the same table/list — `grep -rl` on a couple of distinctive entry names is cheap insurance against exactly this.**
