...
**This content type is structurally different from SKIP/Radicals/Syllables, and the procedure below is still early — validated so far only against 4 hand-picked pages plus 2 genuinely random ones (`五風十雨`, `国士無双`), not a full batch. Expect to keep hardening it the same way `lint Syllables` was (dispatch a small wave, fold in what forks find, repeat) rather than trusting it as final.**
