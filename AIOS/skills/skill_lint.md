...
**This content type is structurally different from SKIP/Radicals/Syllables, and the procedure below is still early — validated so far against 4 hand-picked pages, 2 genuinely random ones (`五風十雨`, `国士無双`), the 6 chronologically-oldest pages (`一刀両断`, `鶏鳴狗盗`, `空中楼閣`, `魑魅罔両`, `開天辟地`, `心性意力`), and a second random-with-constraints batch of 5 spanning all three `origin` categories (`現代適応`, `日用必備` — 単亜語; `戦戦恐恐`, `跳梁跋扈` — classical/ancient; `創反救成` — Bible) — all fixed, not just diagnosed. Still not a full batch. Expect to keep hardening it the same way `lint Syllables` was (dispatch a small wave, fold in what forks find, repeat) rather than trusting it as final.**
