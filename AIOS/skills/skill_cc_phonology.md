...
Confirmed against real data: on `韻 尤`, 9 of the 12 "plain ㄨ" (no-glide) group members are ㄑ/ㄐ-initial — exactly the barred consonants. On `韻 屋三`, all 6 ㄈ-initial characters land on plain `uk`, zero exceptions. `韻 麻三開` (2026-07-10) confirms the ㄑ/ㄐ y-glide ban just as cleanly: all 4 members of its non-glide-bearing group are t͡s/t͡ɕ-initial (→ㄑ/ㄐ), with no affricate-initial character appearing anywhere in the main y-glide group. `韻 微合` initially looked like a counter-example (斐, 扉, 緋 = ㄈㄨㄧ) until re-parsed correctly: ㄨ there is the vowel nucleus with ㄧ as an off-glide coda, not an on-glide — so the rule holds. `韻 戈一合` (2026-07-10) gives a clean confirmation of the ㄋㄌㄅㄆㄇ w-glide ban specifically: every labial-initial (7) and l-initial (2) character on that final lands glide-free on plain ㄚ, with zero exceptions on either side, while a same-final d-initial pair (惰, 堕) — not barred — does carry the glide. `韻 鈬合` and `韻 陽合` (also 2026-07-10) each confirm it again independently — 陽合 splits a perfect 16/8 by labial-vs-non-labial with zero exceptions on either side, the cleanest of the three. The labial w-glide ban is now confirmed on three separate finals — treat it as a reliable, load-bearing rule. **Check this before calling a split "unexplained"** — it may just be phonotactics forcing specific initials out of an on-glide-final group. When checking, be careful to distinguish on-glide (medial, before the vowel) from off-glide (coda, after the vowel) — they look identical in bopomofo notation but only the on-glide is constrained.

### 合 (rounded) finals reliably take a w-glide — a free, ready-made rule

Checked every 開/合 pair in `文法 - 99韻図.md`'s own Vowels-table D column: every single 合 final's Dan'a'yo output contains a w-glide, every 開 counterpart doesn't. Held for all 9 pairs originally checked (泰開/泰合, 寒/桓, 曷/末, 刪開/刪合, 黠開/黠合, 歌/戈一合, 藥開/藥合). **Cite this directly for any 合 final's CJKV Evolution prose** instead of re-deriving it from scratch — but note it is a strong tendency, not exceptionless. Two confirmed counter-examples found later in the sweep (2026-07-10), one in each direction: `韻 佳開` (開 final) has 5 characters (拐, 矮, 柴, 厓, 洒) that unexpectedly carry a w-glide, with no shared conditioning factor among their MC initials; `韻 祭B三合` (合 final) has all 3 of its characters (彗, 穢, 衛) land on plain ㄝ with no w-glide at all. The rule is still worth citing as a strong default, but don't be surprised by an occasional exception on either side.
