...
- **mc_id**: check vault's `lookup/CC/CC 0000.md`, `CC 1000.md`, `CC 2000.md`, and `CC 3000.md` (the corpus is split across all four files, not just `CC 3000.md`) when Wiktionary doesn't list a CC frequency rank — the rank number there IS the mc_id. If absent from Wiktionary and all four CC files (i.e. outside the top ~4000 most-used Classical Chinese characters) **and you are creating the page fresh**, leave `mc_id:` blank — guessing a rank is worse than leaving it empty (this also blocks `date-last-perfect`). Example: 糠 was missed by only checking `CC 3000.md` — it's ranked 2982nd in `CC 2000.md`.
  - **This does not apply when perfecting an existing page that already has a large `mc_id` you can't verify locally** — those four files only mirror the top ~4000 of a fuller external ranking; an unverifiable large number (e.g. `9813`) is real long-tail data, not a fabrication. Never blank it just because your local check came up empty. And `mc_id: 0` is itself a meaningful value ("confirmed absent from the ranking"), never something to "fix" to blank. See [[AIOS/checklists/checklist_characters.md]]'s `mc_id` section for the full policy (added 2026-07-11 after getting this wrong on four pages in one night).
...
- **No attested Middle Chinese ancestry** (modern/folk-coined characters, e.g. 鑫): every other character in the vault has a real `middle_chinese_initial`/`middle_chinese_final`. When one is missing, search for a phonetically near-identical character that *does* have attested MC (same Mandarin/Cantonese/Korean/Japanese/Vietnamese readings) and borrow its MC pair for the frontmatter fields, documenting in bullet 3 that it's a plausible/borrowed reconstruction, not the character's own attested lineage. Confirmed with 武明帥 (2026-07-03, 鑫 case): **do not** then force the new character onto the analog's existing Dan'a'yo syllable — if the Dan'a'yo syllable was already independently fixed (e.g. from the new character's own modern Cantonese/Korean readings, bypassing MC derivation entirely because none exists), keep it as-is. The borrowed MC is for historical-plausibility documentation only; it does not override an already-established syllable assignment. `mc_id` and `date-last-perfect` stay blank as usual for unattested characters.
- **After creating a character**: always update the corresponding syllable page — add the new character under the correct grade heading (Common/Advanced/Naming), update `size` frontmatter, and add the ruby-annotated entry once its legitimizing word exists. If the new character's `graphemic_classification` names an existing vault character as its phonetic component, also add the new character to that parent's `## Derived Characters` section (see [[AIOS/checklists/checklist_characters.md|checklist_characters.md]]) — ruby-annotated with its own syllable, same as a sibling entry for 啖/痰/談 under [[炎]].
