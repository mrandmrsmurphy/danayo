...
```
## Notes
- [穹](../characters/穹.md) "high, sky" + [蒼 (char)](../characters/蒼%20(char).md) "blue, green, grey" — neither character's own `stand_in` points here (穹's is [[穹窿]], 蒼's is [[蒼]] itself), so `#cranberry` does not apply. `kwin: false` per the AND-rule: both constituents are individually `kwin: false`.

**Fixed a real bug**: `羅馬字`/`諺文` had kungcang/쿵창, missing 蒼's own glide (cwang/촹, confirmed against `characters/蒼%20(char).md` and `syllables/ㄑ⺢ㄫ.md`; `注音` ㄎㄨㄫㄑ⺢ㄫ was already correct) — corrected to kungcwang/쿵촹.
