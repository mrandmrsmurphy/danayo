...
```
## Notes
- [空 (char)](../characters/空%20(char).md) "empty" + [虚 (char)](../characters/虚%20(char).md) "void, emptiness" — neither character's own `stand_in` points here (both 空's and 虚's own are themselves), so `#cranberry` does not apply. `kwin: false` per the AND-rule: 空 is individually `kwin: false`.

**Fixed a real bug**: `vietnamese` had trống không, a native Vietnamese compound unrelated to either constituent's own Sino-Vietnamese reading — corrected to không hư (空's own không + 虚's own hư), matching this page's own body analysis below.
...
The term bridges Buddhist and Daoist conceptions of emptiness, though these traditions understand emptiness somewhat differently. <ruby>佛教<rt>ㄅㄨㄊㄍ⼘ㄨ</rt></ruby> emphasizes the lack of inherent existence (_svabhāva_), while Daoism emphasizes the productive potential of emptiness and non-being.
...
**Korean (공허, _gongheo_)**: Similar to Japanese, preserves the term in both philosophical contexts (particularly in discussions of <ruby>佛教<rt>ㄅㄨㄊㄍ⼘ㄨ</rt></ruby>) and everyday usage meaning "empty," "hollow," or psychologically "void."
