...
深淵, composed of 深 ("deep") and 淵 ("abyss," "pool," or "depths"), denotes a deep abyss, profound depths, or bottomless pool. The term carries both literal geographical meaning and rich metaphorical significance in Chinese philosophical, cosmological, and literary traditions. This word is itself the stand-in that legitimizes the character 淵 as an independent Dan'a'yo entry (深's own stand-in is [[深刻]], so no #cranberry: only one side of the pair is a bound character).
...
**Mandarin (shēnyuān) and Cantonese (sam1 jyun1)**: fully compositional, matching 深's and 淵's own stored citations exactly.
...
Fixed `pos` from 固有名詞 to 名詞 (淵 is a common noun on its own character page, and 深淵 is not a proper noun) and lowercased `english` from "Abyss" to "abyss" for consistency. Filled the blank `vietnamese` field per the prose above. No homophones (注音 ㄙㄧㄇ⼔ㄋ unique across both words and characters).
