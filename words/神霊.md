...
```
## Notes
- [神 (char)](../characters/神%20(char).md) "god, deity, spirit" + [霊](../characters/霊.md) "spirit, ghost" — neither character's own `stand_in` points here (神's is [[神]] itself, 霊's is [[霊魂]]), so `#cranberry` does not apply. `kwin: false` per the AND-rule: 霊 is individually `kwin: false`.
