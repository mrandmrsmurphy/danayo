### 2026-07-26, iteration 447 — [[characters/宗|宗]]

Next never-perfected character by `danayo_id` (3083). Stamped `date-last-perfect: 2026-07-26`. `graphemic_classification: 會意` already correct — verified via Wiktionary: 会意 of [[Radical 040|宀]] ("house, roof") and [[示]] ("spirit tablet, altar") — depicting an ancestral temple/shrine. Possible Proto-Sino-Tibetan link to "to build," cognate with Tibetan *rdzong* ("castle") and Burmese *hcaung* ("room/hall"); the administrative-division sense (as in Tibetan "dzong") is a separate loan, only distantly related to the native "ancestral temple" sense. `mc_id: 285` cross-checked against `lookup/CC/CC 0000.md` — exact match.

**Frontmatter**: `pos: ''` (empty string) → filled in as `名詞`, matching the stand-in word `words/宗教.md`'s own field.

**Body defects found**: `# Notes` used H1 with no graphemic bullet at all, and `## Words` was placed before it; two Words entries (宗師, 宗教) were bare wikilinks, one with a dash-separated gloss; the two CC initial/final links were stranded mid-document; SKIP/Stroke, MC rank, and Levels bullets were all missing; 1 of 5 ground-truth words missing; no `## Derived Characters` despite two real hits.

**Words cross-check** (5 total ground-truth hits): 宗徒, 世宗 already correctly ruby'd, kept; 宗教 (stand-in), 宗師 reformatted to ruby+gloss; 1 missing — 宗派 ("faction; sect; clan") — added from stored fields.

**Chengyu**: no ground-truth hits — section correctly omitted.

**Derived Characters** (2 total ground-truth hits — characters naming 示 as their `graphemic_classification`): 視 (char) ("look at; inspect," pipe-linked), 祁 ("surname") — both added.
...
### 2026-07-26, iteration 446 — [[characters/安|安]]

Next never-perfected character by `danayo_id` (3082). **Frontmatter defect found and fixed, with a genuinely subtle etymology**: `graphemic_classification: 會意` contradicted the page's own pre-existing bullet, which already stated "会意 of 宀+女, a woman at rest under a roof." Fetched Wiktionary's raw wikitext directly to be certain before touching this one, since 女 (nǚ) doesn't phonetically resemble 安 (ān) in any modern daughter language — and the source confirms 安 is actually **形声**: semantic 宀 ("roof") + phonetic 女, but critically, this 女 is *not* the ordinary "woman" morpheme — it's a distinct, now-obsolete homograph glossed "a seated person" (sitting with legs together, buttocks on the heels, OC \*ʔˤan, with its own extra stroke depicting the other leg, explicitly not decorative), matching 安's own OC \*qaːn. The popular "woman resting under a roof" story doesn't appear on Wiktionary at all — it's an outside folk gloss, not something the source states or debunks. Corrected the field to `女`, with the bullet carefully qualifying which 女 is meant so a future reader isn't misled the way the old bullet's "a woman at rest" phrasing would mislead. Stamped `date-last-perfect: 2026-07-26`. `mc_id: 105` cross-checked against `lookup/CC/CC 0000.md` — exact match (top-110 most frequent character in the corpus). `pos: 性詞` was already correct.

**Body defects found**: bullets 2–4 were compressed and reordered — Levels appeared before SKIP/Stroke, and the MC-rank bullet used "·" separators cramming SKIP/Stroke/syllable/initial/final together — split back into the canonical four-bullet sequence; 8 of 22 ground-truth words missing, including the stand-in 平安 itself and an entire greetings family (今日安/今朝安/今昼安/今晩安/今夜安); several existing entries were bare wikilinks with no ruby; no `## Derived Characters` despite four real hits.

**Words cross-check** (22 total ground-truth hits — the largest single Words list found this loop): 安静, 安逸, 安土, 不安, 治安, 安心 already correctly ruby'd, kept; 安康魚, 安楽, 安価, 安全, 安慰, 安穏, 安寧, 安置 reformatted from bare wikilinks to ruby+gloss; 8 missing — 平安 (stand-in), 安定 ("stable; settled"), 慰安 ("comfort, console"), 今日安 ("hello"), 今朝安 ("good morning"), 今昼安 ("good day"), 今晩安 ("good evening"), 今夜安 ("good night") — all added from stored fields.

**Chengyu**: 1 ground-truth hit — 安心立命 — already present, kept.

**Derived Characters** (4 total ground-truth hits — characters naming 安 as their `graphemic_classification`): 按 ("put hands on"), 晏 ("late"), 案 ("propose; suggest; motion"), 鞍 ("saddle") — all four added.

**Verification**: Python cross-check of all 27 `<rt>` values (22 Words + 1 Chengyu + 4 Derived Characters) against each cited file's own `注音` field — 0 mismatches.

### 2026-07-26, iteration 445 — [[characters/宇|宇]]

Next never-perfected character by `danayo_id` (3081). Stamped `date-last-perfect: 2026-07-26`. `graphemic_classification: 于` already correct — verified via Wiktionary: 形声 (OC \*ɢʷaʔ), semantic [[Radical 040|宀]] ("roof") + phonetic [[于 (char)|于]]. Origin flagged as uncertain by the source itself, with a possible alternate tie to 羽 ("wings," i.e. "wings of a house") or a stem related to 況; semantic drift runs "eaves" → "abode, estate, territory" → "space" → "universe." `mc_id: 1476` cross-checked against `lookup/CC/CC 1000.md` — exact match.

**Frontmatter**: `pos: ''` (empty string) → filled in as `名詞`; `words/宇宙.md` had a blank `pos` field too, with nothing to inherit from.

**Body defects found**: the SKIP/Stroke, MC rank, and Levels bullets were all missing outright, with the two CC initial/final links floating at the top instead; the stand-in 宇宙 itself was missing from Words; no `## Chengyu` despite a real hit; no `## Derived Characters` despite four real hits.

**Words cross-check** (5 total ground-truth hits): 宇宙人, 宇宙局, 宇宙観 already correctly ruby'd, kept; 2 missing — 宇宙 (stand-in), 宇宙船 ("spaceship") — added from stored fields.

**Chengyu**: 1 ground-truth hit — 八紘一宇 ("Manifest Destiny of Japan") — added.

**Derived Characters** (4 total ground-truth hits — characters naming 于 as their `graphemic_classification`): 芋 ("taro"), 汚 (char) ("dirty," pipe-linked), 盂 ("cup; basin"), 迂 ("roundabout; circuitous") — all four added.
